...
# pg-e2e: the real-Postgres e2e suite (CAIRN_E2E_PG=1, --test-threads=1 — see
...
# Uses the `cairn` superuser (not CAIRN_PG_URL_DEFAULT's least-privilege
# cairn_writer): the tests create slots/publications and TRUNCATE.
CAIRN_E2E_PG_URL ?= postgres://cairn:cairn@localhost:5433/cairn
.PHONY: pg-e2e
pg-e2e: ## Real-Postgres e2e suite; drops leaked inactive e2e_* slots first.
	...
	CAIRN_E2E_PG=1 CAIRN_PG_URL=$(CAIRN_E2E_PG_URL) $(CARGO) test -p cairn-infra --features pg --no-fail-fast -- --test-threads=1
# cairn-cli's pg suite too: `cairn link --mode direct` generates SQL, and the
...
# mis-scoping) it. e2e_pg_direct_sql owns the `cairn` schema, hence -threads=1.
	CAIRN_E2E_PG=1 CAIRN_PG_URL=$(CAIRN_E2E_PG_URL) $(CARGO) test -p cairn-cli --no-fail-fast -- --test-threads=1
...
# `supabase/` project dir; override it.
	...
	CAIRN_SB_ANON_KEY=$$(cd $$SB_DIR && supabase status -o json | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>console.log(JSON.parse(s).ANON_KEY))") \
	CAIRN_SB_SERVICE_KEY=$$(cd $$SB_DIR && supabase status -o json | node -e "let s='';process.stdin.on('data',d=>s+=d).on('end',()=>console.log(JSON.parse(s).SERVICE_ROLE_KEY))") \
	node scripts/e2e-supabase-direct.mjs

.PHONY: web-conformance
web-conformance: ## The browser-Worker leg of cairn_core::conformance (OPFS, headless Chromium).
...
# rebuilt straight after.
	wasm-pack build crates/cairn-ffi-wasm --target web --out-dir pkg-web --features conformance
	cd sdk/cairn_web && npx playwright test e2e/conformance.spec.cjs --reporter=line
	wasm-pack build crates/cairn-ffi-wasm --target web --out-dir pkg-web
