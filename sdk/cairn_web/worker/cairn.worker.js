...
// export — a static `import { cairnConformance }` would fail to link against
// the production bundle.
import * as wasm from "../pkg-web/cairn_ffi_wasm.js";
...
async function initStorage() {
  if (!(leaderLockHeld || (await acquireLeaderLock()))) {
    ...
    console.warn("[cairn.worker] another tab owns the durable store; memory mode");
  } else {
    try {
...
// Rc<RefCell<...>>; CairnSocket.connect resumes from the durable checkpoint.
async function reconnect() {
  ...
  await ensureWasm();
  sock = await CairnSocket.connect(rp.url, token, rp.table, rp.where_sql, dbHandle);
...
self.onmessage = async (ev) => {
  ...
  // (`cairn_core::conformance`). Runs against whatever backend this Worker
  ...
  if (m.cmd === "conformance") {
    await bootP;
    if (typeof wasm.cairnConformance !== "function") {
      self.postMessage({
        id: m.id,
        error:
    ...
    try {
      const cases = wasm.cairnConformance(dbHandle ?? undefined);
  ...
  try {
    switch (m.cmd) {
      ...
      case "wake": {
        ...
        // cairn.sw.js → client.postMessage("cairn:wake") → the page forwards
