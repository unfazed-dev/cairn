...
// Same shape as browser_live.spec.cjs: the spine prints CAIRN_E2E_PORT= then
// CAIRN_E2E_READY on stdout.
function startSpine() {
  return new Promise((resolve, reject) => {
    if (!fs.existsSync(SPINE_EXE)) {
      reject(new Error("spine binary not found at " + SPINE_EXE + " — run `cargo build -p cairn-infra --example e2e_server` first."));
      return;
    }
...
test("multi-tab: follower proxies to the leader, opt-out stays standalone, follower is promoted when the leader closes", async ({
  ...
  try {
    ...
    const c1 = await tab1.page.evaluate((u) => window.cairn.connect(u, null, "tasks", null), wsUrl);
    expect(c1.ok).toBe(true);
    const c2 = await tab2.page.evaluate((u) => window.cairn.connect(u, null, "tasks", null), wsUrl);
    expect(c2.ok).toBe(true);

    // Write in the follower → visible in the leader: one engine, one store.
    const pk = "multitab-" + Date.now();
    await tab2.page.evaluate(
      (pk) => window.cairn.write("tasks", "upsert", pk, JSON.stringify({ title: "from follower" }), "w-" + pk),
      pk,
    );
    const pksIn = (tab) => async () => {
      const r = await tab.page.evaluate(() => window.cairn.rowsFor("tasks"));
    ...
    const cp = await tab2.page.evaluate(() => window.cairn.checkpoint());
    ...
    const c3 = await tab3.page.evaluate((u) => window.cairn.connect(u, null, "tasks", null, true), wsUrl);
    expect(c3.ok).toBe(true);
    await tab3.page.evaluate(() => window.cairn.close());
    ...
    await expect
      .poll(
        () =>
          tab2.page.evaluate(() =>
            window.cairn.events.some((e) => e.type === "storage" && e.mode === "durable" && !e.reason),
          ),
    ...
    await tab2.page.evaluate(() => window.cairn.close());
