...
// @cairn/web tarball with every JS file and no engine — it installs fine and
// fails at runtime. `build:web` and `prepack` drop an empty `.npmignore` there;
// this asserts the outcome rather than the mechanism.
//
// Run: node check-pack.cjs   (from sdk/cairn_web)

const { execFileSync } = require("node:child_process");

const REQUIRED = [
  "pkg-web/cairn_ffi_wasm_bg.wasm",
  "pkg-web/cairn_ffi_wasm.js",
  "worker/cairn.worker.js",
  "index.js",
];
...
if (missing.length) {
  console.error("MISSING from the @cairn/web tarball:\n  " + missing.join("\n  "));
