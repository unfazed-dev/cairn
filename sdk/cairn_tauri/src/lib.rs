...
impl CairnPluginConfig {
    ...
    /// else the `"tasks"` floor (the same default `sdk/cairn_node` pins).
...
mod tests {
    ...
    async fn plugin_config_tables_lifts_the_one_table_ceiling() {
        let config: CairnPluginConfig = serde_json::from_value(serde_json::json!({
