//! Direct mode's Flutter surface: the same device engine, with no Cairn server
//! on the other end of it.
//!
//! [`crate::api::cairn::CairnHandle`] dials a `/sync` WebSocket that somebody
...
//!   [`CairnDirectHandle::increment`].
//!
//! The rest — storage, apply, watch, the durable outbox — is byte-for-byte the
//! server-mode engine, because it is the same `cairn-core`.

use std::sync::Arc;

use cairn_client::sqlite::ClientTable;
use cairn_client::{DirectClient, SqliteStorage};
...
use crate::api::cairn::{
    CairnConnectionState, CairnWriteInput, ClientTableFfi, WriteQueueStatusFfi,
};
...
impl Drop for CairnDirectHandle {
    fn drop(&mut self) {
        self.abort_run();
        self.abort_pumps();
    }
...
impl CairnDirectHandle {
    ...
    pub fn connect(
        ...
        token: Option<String>,
        db_path: String,
    ) -> Result<CairnDirectHandle, String> {
        let rt =
    ...
    pub async fn write(
        ...
        let ids = self.enqueue(vec![CairnWriteInput {
            table,
            op,
            pk,
            payload_json,
    ...
    /// is touched.
    pub async fn write_batch(&self, ops: Vec<CairnWriteInput>) -> Result<Vec<u64>, String> {
    ...
    fn spawn_run(
        ...
        *slot = Some(self.rt.spawn(async move {
            ...
            eprintln!("cairn direct: sync loop stopped: {fatal}");
        }));
        Ok(())
    }

    /// Validate and enqueue, without touching the network.
    fn enqueue(&self, ops: Vec<CairnWriteInput>) -> Result<Vec<u64>, String> {
