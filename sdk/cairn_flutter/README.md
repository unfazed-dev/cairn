...
`CairnDatabase` and rides the normal resume path (task 5.1's non-destructive
...
`CairnPushTokenException` (non-`204` reply). Tokens registered before a
...
- `CairnDatabase.local({required String sqliteDir, required CairnSchema schema, Set<String>? orSetTables, Set<String>? counterTables})`
...
| Web | Supported (ADR-0036): the conditional import selects `WebCairnEngine`, which drives the *shared* `cairn-ffi-wasm` Worker (`web/cairn/cairn_worker.js`) with an opfs-sahpool SQLite store — durable, no COOP/COEP, Safari ≥ 16.4. Apps drop `cairn_worker.js` + `sqlite_wasm_glue.js` + `cairn_ffi_wasm.{js,_bg.wasm}` + `@sqlite.org/sqlite-wasm` under `web/` (see ADR-0036 §4); build as JS (`flutter build web`, not `--wasm`). One durable tab per origin: a second tab is refused at `connect` (`CairnWebStorageMode.secondaryTab`, Web Lock leader guard, 2026-09-21). `sqlitePath` is ignored; `LocalFileBlobStore` is `dart:io` — supply your own `BlobStore` for attachments. Browser round-trip pinned by `web/e2e/flutter_web_smoke.spec.cjs`. |
...
- `example/integration_test/cairn_server_test.dart` — the real end-to-end
  ...
  (default 15min; raise via `CAIRN_IT_COMPILE_BUDGET_MIN` on slow storage —
  ...
  The spawned server also gets `CAIRN_RULES_FILE` pointed at an explicit
  all-mode temp file — the server resolves `cairn_rules.toml` from its
...
build hook, the live-`cairn-server` macOS integration test green, and the
...
| pin | value | why exact |
|---|---|---|
| `cairn_flutter` | `0.2.0-dev.1` (path dep until first pub.dev publish) | pre-release head; see CHANGELOG |
...
`sdk/` tree — no git tag has ever carried this package. "Track cairn's
...
- `lib/adapters/cairn_adapter.dart` — side-by-side with
...
- `lib/push/push_pilot.dart` — the push reference: FCM mobile + VAPID web,
  ...
  `cairnDoorbellBackgroundHandler` background-isolate wake pattern (an FCM
