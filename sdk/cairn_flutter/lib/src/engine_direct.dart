/// The direct-mode [CairnEngine] adapter: wraps the generated
/// `rust.CairnDirectHandle`, which talks to a Supabase project with no
/// `cairn-server` anywhere (ADR-0045).
///
...
/// share a body).
///
/// Three verbs of [CairnEngine] have no meaning without a server, and they
...
class DirectCairnEngine implements CairnEngine {
  DirectCairnEngine._(this._handle, this._scope, this._counterFields);
  ...
  /// that column from `CAIRN_COUNTER_COLUMNS`, and direct mode has no server to
  /// read it from, so the app declares it here.
  factory DirectCairnEngine.connect({
    ...
    direct.CairnDirectHandle.connect(
  ...
  final direct.CairnDirectHandle _handle;
  final String _scope;
  ...
  Stream<CairnConnectionState> subscribe({
    ...
    if (filtered.isNotEmpty) {
      throw UnsupportedError(
        ...
        'Express it in cairn_rules.toml and re-run `cairn link --mode direct`.',
      );
    }
  ...
  Future<String> subscribeStream({
    ...
    '(cairn_rules.toml [streams.$name]) and there is no server to hold them.',
  );
  ...
  Future<int> _increment(String table, String pk, int delta) async {
    ...
    if (field == null) {
      throw StateError(
        ...
        'counterFields: {"$table": "<column>"} to DirectCairnEngine.connect. '
...
const String _noOrSet =
    ...
    'as rows and delete them, or run a cairn-server for this table.';
