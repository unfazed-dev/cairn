...
class Cairn {
  ...
  /// `cairn dev`'s printed URL), or an `iroh://…?ticket=…` dial URL when the
  ...
  /// Open a connection with no `cairn-server` in it: this device talks to
  ...
  /// decide what a device can see. Run `cairn doctor --mode direct` to check
  /// them.
  ///
  ...
  /// from `CAIRN_COUNTER_COLUMNS`; there is no server here to read it from).
  ///
  /// Native-only today; on web this throws [UnsupportedError].
  static Future<Cairn> direct({
