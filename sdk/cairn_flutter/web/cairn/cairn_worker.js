...
// refuses `connect` (Dart surfaces it as CairnWebStorageMode.secondaryTab).
// Mirrors sdk/cairn_web/worker/cairn.worker.js — keep the two in step.
...
async function initStorage() {
  if (!(await acquireLeaderLock())) {
    ...
    console.warn("[cairn_worker] another tab owns the durable store; memory mode");
  } else {
    try {
...
async function openSocket() {
  ...
  if (schemaTables !== null) {
    try {
      ...
      console.warn("[cairn_worker] applySchema on connect failed:", e);
    }
  }
  ...
  try {
    switch (m.cmd) {
      ...
      case "counterDecrement": {
        // Wave 4c (ADR-0036): CRDT delegates on CairnSocket. Each mints a
        // client HLC + enqueues + apply_locals in the engine (reusing
        // cairn-domain), then ships if OPEN. The wasm method name matches the
      ...
      case "writeBatch": {
        ...
        // ops is [{table, op, pk, payloadJson?}, ...] → matches CairnEngine's
      ...
      case "setCrdtTables": {
        ...
        // merges instead of clobbering. Delegates to CairnSocket.setCrdtTables
