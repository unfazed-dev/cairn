...
group = providers.gradleProperty("cairnGroupId").getOrElse("io.github.unfazed-dev")
version = providers.gradleProperty("cairnVersion").getOrElse("0.2.0")
...
//
//   ./gradlew centralBundle -PcairnGroupId=io.github.you \
//       -PsigningInMemoryKey="$(gpg --armor --export-secret-keys KEYID)" \
//       -PsigningInMemoryKeyPassword=…
//   → android/build/distributions/cairn-kotlin-<version>-central-bundle.zip
//
...
publishing {
    publications {
        register<MavenPublication>("release") {
            artifactId = "cairn-kotlin"
            // AGP's `release` component only exists after evaluation.
            afterEvaluate { from(components["release"]) }
            pom {
                name.set("cairn-kotlin")
                description.set(
                    "Kotlin/Android SDK for Cairn — local-first sync over a Rust core (UniFFI).",
                )
                url.set("https://github.com/unfazed-dev/cairn")
                licenses {
                    license {
                ...
                developers {
                    developer {
                        id.set("unfazed-dev")
                        name.set("Cairn maintainers")
                        url.set("https://github.com/unfazed-dev")
                    }
                }
                scm {
                    url.set("https://github.com/unfazed-dev/cairn")
                    connection.set("scm:git:https://github.com/unfazed-dev/cairn.git")
                    developerConnection.set("scm:git:ssh://git@github.com/unfazed-dev/cairn.git")
                }
            }
        }
    }
...
tasks.register<Zip>("centralBundle") {
    ...
    archiveFileName.set("cairn-kotlin-$version-central-bundle.zip")
