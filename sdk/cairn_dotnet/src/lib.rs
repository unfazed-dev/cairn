...
pub struct CairnClient {
    ...
    /// user). `StdMutex` next to `token` (never held across an await): the
    /// UniFFI receiver is `&self`, like `cairn_node`'s identical field.
    ///
...
impl CairnClient {
    ...
    pub fn sign_out(&self) -> Result<(), CairnError> {
        self.rt.block_on(async {
            // ADR-0037 §3: the sign-out deregistration (step 5) needs the JWT
            // from BEFORE step (4) clears it — capture it now.
            ...
            *self.token.lock().expect("token lock poisoned") = None;
            // (5) ADR-0037 §3: deregister this session's push tokens —
            ...
            // step (5). Uses the token captured before (4) cleared it.
...
mod tests {
    ...
    // ─────────── push-token REST (ADR-0037 §3 / plan task 5.4) ───────────
    ...
    fn register_push_token_posts_exact_json_with_bearer() {
        ...
        assert!(
            lower.contains("authorization: bearer dotnet-jwt"),
    ...
    fn register_push_token_unknown_platform_is_an_error() {
        ...
        let client = push_client(&authority, Some("dotnet-jwt"));
        let err = client
            .register_push_token("wns".into(), "tok-1".into())
    ...
    /// must carry the JWT captured BEFORE sign_out clears the stored seed
