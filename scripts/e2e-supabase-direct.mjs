...
// ## Why this exists
//
// `crates/cairn-cli/tests/e2e_pg_direct_sql.rs` proves the generated SQL
...
// so the same script runs against a hosted project (`CAIRN_SB_URL=https://…`),
// where `CAIRN_SB_PSQL` is the one thing that has to change.

import { execFileSync } from "node:child_process";

const URL_BASE = process.env.CAIRN_SB_URL ?? "http://127.0.0.1:54321";
const ANON = process.env.CAIRN_SB_ANON_KEY;
const SERVICE = process.env.CAIRN_SB_SERVICE_KEY;
...
const PSQL = (process.env.CAIRN_SB_PSQL ??
...
const PUSH_ENDPOINT = process.env.CAIRN_SB_PUSH_ENDPOINT ??
  "http://host.docker.internal:54321/functions/v1/cairn-push";

if (!ANON || !SERVICE) {
  console.error("set CAIRN_SB_ANON_KEY and CAIRN_SB_SERVICE_KEY (supabase status -o json)");
  process.exit(2);
}
...
async function signUp(email) {
  const res = await fetch(`${URL_BASE}/auth/v1/signup`, {
    ...
    body: JSON.stringify({ email, password: "cairn-e2e-password" }),
  });
...
const alice = await signUp(`alice-${Date.now()}@cairn.test`);
const bob = await signUp(`bob-${Date.now()}@cairn.test`);
...
await section("xid8 over the wire", async () => {
  const r = await rest("/cairn_e2e_notes", {
    token: alice.token,
...
await section("rls", async () => {
  ...
  const after = await pullUntil(bob.token, (rows) =>
    rows.some((r) => r.table_name === "cairn_e2e_catalog"));
  check(
    "an unscoped table reaches every authenticated device",
    Array.isArray(after.body) && after.body.some((r) => r.table_name === "cairn_e2e_catalog"),
...
await section("increment", async () => {
  ...
  for (let i = 0; i < 3; i += 1) {
    const r = await rest("/rpc/cairn_increment", {
      token: alice.token,
      body: { p_table: "cairn_e2e_notes", p_pk: id, p_field: "hits", p_delta: 1 },
    });
    if (r.status >= 300) throw new Error(`increment ${i}: HTTP ${r.status} ${JSON.stringify(r.body)}`);
  }
  const row = (await rest(`/cairn_e2e_notes?id=eq.${id}&select=hits`, {
    token: alice.token,
    method: "GET",
  })).body[0];
  ...
  const notMine = await rest("/rpc/cairn_increment", {
    token: bob.token,
    body: { p_table: "cairn_e2e_notes", p_pk: id, p_field: "hits", p_delta: 100 },
  });
  const still = (await rest(`/cairn_e2e_notes?id=eq.${id}&select=hits`, {
    token: alice.token,
    method: "GET",
  })).body[0];
  check(
...
await section("scope change", async () => {
  ...
  sql(`update public.cairn_e2e_notes set owner_id = '${bob.sub}' where id = '${id}';`);
...
await section("retention", async () => {
  ...
  check(
    ...
    ["cairn_e2e_notes", "cairn_e2e_catalog"].every((tbl) =>
  ...
  check(
    ...
    snap.body
      .filter((r) => r.table_name === "cairn_e2e_notes" && r.row)
...
await section("doorbell", async () => {
  ...
  // The reason `cairn doctor --mode direct` reports the dashboard toggle it
...
await section("push", async () => {
  ...
  // The check this whole section exists for. `cairn` is not an exposed schema,
