...
area_e2e_pg() {
  ...
  # The CI job's cairn-infra --features pg suite, plus cairn-cli's pg suite
...
area_flutter() {
  ...
  python3 sdk/cairn_flutter/scripts/check-doc-signatures.py
  (
    cd sdk/cairn_flutter/rust
...
area_sdk_typecheck() {
  ...
  (cd sdk/cairn_react_native && npm ci && npm run typecheck)
  (cd sdk/cairn_capacitor && npm ci && npm run build)
}
