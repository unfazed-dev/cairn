#!/usr/bin/env bash
# =============================================================================
# rename-to-qairn.sh — the cairn → qairn sweep.
#
...
# mid-flight (a rename of crates/cairn-infra/ moves 59 not-yet-rewritten files
...
#   CAIRN → QAIRN   (env vars, feature flags, Make variables)
#   Cairn → Qairn   (prose, Dart/Swift/Kotlin/C# type names)
#   cairn → qairn   (crate names, snake paths, urls, npm scope, reverse-DNS)
# The three are disjoint under case-sensitive matching, so the sweep is one
# pass and is naturally IDEMPOTENT: "qairn" contains no "cairn", so a second
...
FROM_LC="cairn"; TO_LC="qairn"
FROM_TC="Cairn"; TO_TC="Qairn"
FROM_UC="CAIRN"; TO_UC="QAIRN"
...
sweep because its identity is owned by something outside this repository.
  ...
  --pg-identity        the Postgres role / password / database name / SQL
                       object names (cairn:cairn@…/cairn, cairn_writer,
  ...
  --client-storage     on-device names: cairn.db / cairn.sqlite, the client
                       ...
                       prefix "cairn:checkpoint:", the .cairn/ project dir and
                       cairn_rules.toml. Renaming orphans existing local data
                       unless a migration ships with it.
  --external-ids       things already published under the old name: the GitHub
                       repo URL, the homebrew-cairn tap, v0.1.0/v0.2.0 release
                       asset filenames, the dev.cairn.* bundle ids.
  ...
  --brand-prose        the five sentences that explain what a cairn IS (a pile
...
if [[ $OPT_PG_IDENTITY -eq 0 ]]; then
  ...
  p pg-identity 'POSTGRES_(?:USER|PASSWORD|DB):[ \t]*cairn\b'
  p pg-identity '(?:-U|-d)[ \t]+cairn\b'
  p pg-identity '\bcairn_writer(?:_dev_pw)?\b'
  p pg-identity '\bidx_cairn_\w+'
  p pg-identity '\bcairn_(?:pub|oplog|slot|push_tokens)\b'
  p pg-identity 'CREATE ROLE cairn\b'
fi

if [[ $OPT_WIRE_IDENTITY -eq 0 ]]; then
  p wire-identity 'cairn/sync/1'
  p wire-identity 'plugin:cairn\|'
  p wire-identity 'plugins\.cairn\b'
...
if [[ $OPT_CLIENT_STORAGE -eq 0 ]]; then
  ...
  p client-storage '\bcairn_(?:data|meta|outbox)\b'
  p client-storage '\bcairn\.(?:db|sqlite|toml)\b'
  p client-storage '\bcairn_rules\.toml\b'
  p client-storage '\.cairn(?:/|")'
fi

if [[ $OPT_EXTERNAL_IDS -eq 0 ]]; then
  p external-ids 'https://github\.com/[^\s"'"'"'`)<]*cairn[^\s"'"'"'`)<]*'
  p external-ids '\bhomebrew-cairn\b'
  p external-ids '\bdev\.cairn[\w.]*'
  p external-ids '\bcairn-(?:aarch64|x86_64)-[\w.-]+?\.(?:tar\.gz|zip)(?:\.sha256)?'
fi
...
if [[ $OPT_BRAND_PROSE -eq 0 ]]; then
  ...
  p brand-prose '[Aa] cairn is a pile of stones[^.]*\.'
  p brand-prose '[Aa] cairn is a trail marker of stacked stones[^.]*\.'
  p brand-prose '[Aa] cairn is a stack of stones[^.]*\.'
...
# the bytes that get substituted. Shapes overlap on purpose (dev.cairn and
# @cairn/ are both also lowercase `cairn`) — the report says so.
my @cats = (
  ['SCREAMING  CAIRN_*'      => qr/CAIRN/      ],
  ['TitleCase  Cairn*'       => qr/Cairn/      ],
  ['snake      cairn_*'      => qr/cairn_/     ],
  ['kebab      cairn-*'      => qr/cairn-/     ],
  ['reverseDNS dev.cairn*'   => qr/dev\.cairn/ ],
  ['npm scope  @cairn/*'     => qr/\@cairn\//  ],
  ['bare       cairn'        => qr/cairn(?![-_])/ ],
);
...
for my $path (@files) {
  ...
  my $orig = <$fh>; close $fh;
  next unless defined $orig && $orig =~ /cairn/i;
  ...
  if ($mode eq 'apply') {
    ...
    for my $i (0 .. $#o) {
      ...
      for my $c (@cats) {
        ...
        # line that only changed CAIRN_PG_SLOT gets sampled as a `cairn_` change
...
print "  (shapes overlap: dev.cairn and \@cairn/ are also counted as lowercase)\n\n";
...
# each move, because renaming crates/cairn-infra/ shifts every path beneath it.
...
list_dirs_to_move() {
  git ls-files | while IFS= read -r f; do
    ...
    while d="$(dirname "$d")"; [[ "$d" != "." ]]; do
      case "$(basename "$d")" in *[Cc]airn*|*CAIRN*) echo "$d" ;; esac
    done
...
if [[ "$MODE" == "apply" ]]; then
  ...
  SKIPPED_PATHS=$(grep -ci cairn "$EXCLUDED" 2>/dev/null || true)
  echo "  $(grep -ci cairn "$EXCLUDED" 2>/dev/null || echo 0) excluded paths contain 'cairn' and stay put"
fi
...
==============================================================================
 ...
 AFTER --apply, these are NOT done by this script and must be done by hand:
   ...
   * rebuild the committed wasm blob (apps/atlet/flutter/web/cairn/*.wasm
