...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /// CairnAdapter.setToken for what that costs.
  ...
  void initState() {
    super.initState();
    // Direct-mode Cairn is the only engine: bring it up on entering Home so
  ...
  /// (user request 2026-09-22): direct-mode Cairn, the device syncing with
  /// Supabase itself with no `cairn-server` on the other end (ADR-0045).
    ...
    try {
      ...
      _authSub = Supabase.instance.client.auth.onAuthStateChange.listen((s) {
        ...
        if (s.event == AuthChangeEvent.tokenRefreshed ||
          ...
          unawaited(cairn.setToken(token).catchError((Object _) {}));
        }
      });
      // PILOT (ADR-0037): doorbell registration follows the cairn engine —
      // push is a cairn feature, and direct mode is still a CairnAdapter.
      if (_pushPilotEnabled) {
  ...
  /// server-mode and direct-mode Cairn adapters. Deliberately bypasses
  ...
  Future<void> _runBenchSuite(BenchStore store) async {
    ...
    await runFullSuiteForEngines(
      ...
      adapterFactories: {
        ...
        Engine.cairnDirect: () =>
            CairnAdapter.direct(anonKey: _supabaseAnonKey),
      },
    );
  }
