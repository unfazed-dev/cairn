...
/// A cairn doorbell: server mode's `{table, lsn}` (ADR-0037 §2), or direct
...
Future<void> cairnDoorbellBackgroundHandler(RemoteMessage message) async {
  ...
  try {
    ...
    final db = await openCairnDirect(
  ...
  Future<void> attach(CairnAdapter adapter) async {
    ...
    if (kIsWeb) {
      try {
        ...
        // Covers CairnPushTokenException (server rejected) and the browser
