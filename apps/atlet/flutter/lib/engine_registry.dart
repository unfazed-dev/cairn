...
/// Which Cairn client is live. `cairnDirect` is the same engine with no
/// `cairn-server` on the other end — the device syncs with Supabase itself
...
/// in fakes without touching the real Cairn SDK.
class EngineRegistry {
  EngineRegistry({
    SyncAdapter Function()? cairnFactory,
    SyncAdapter Function()? cairnDirectFactory,
    String supabaseAnonKey = '',
  }) : _cairnFactory = cairnFactory ?? (() => CairnAdapter()),
       _cairnDirectFactory =
           cairnDirectFactory ??
           (() => CairnAdapter.direct(anonKey: supabaseAnonKey));

  final SyncAdapter Function() _cairnFactory;
  final SyncAdapter Function() _cairnDirectFactory;

  SyncAdapter? _cairnAdapter;
  SyncAdapter? _cairnDirectAdapter;
  ...
  SyncAdapter? get current => switch (_activeEngine) {
    ...
    Engine.cairnDirect => _cairnDirectAdapter,
    null => null,
  };
  ...
  Future<SyncAdapter> start(Engine engine, SyncSession session) async {
    ...
    final adapter = switch (engine) {
      Engine.cairn => _cairnFactory(),
      Engine.cairnDirect => _cairnDirectFactory(),
    };
