...
class CairnAdapter implements SyncAdapter {
  /// Server mode: sync through a `cairn-server` `/sync` socket.
  CairnAdapter() : engine = 'cairn', _open = _openServer;

  /// Direct mode: sync straight with Supabase, no `cairn-server` anywhere
  ...
  CairnAdapter.direct({required String anonKey})
    : engine = 'cairn-direct',
      _open =
          (({
  ...
  /// mode do differently.
  final Future<CairnDatabase> Function({
    required String supabaseUrl,
    required String accessToken,
    required String userId,
    required String dbDir,
  ...
  static Future<CairnDatabase> _openServer({
    required String supabaseUrl,
    required String accessToken,
    required String userId,
    required String dbDir,
  }) => CairnDatabase.connect(
    url: _cairnUrl,
    token: accessToken,
    schema: _schema,
  ...
  Future<void> init({
    ...
    await db.subscribeTables(const [
      ...
      CairnTableSub(name: 'order_events'),
    ]);
  ...
  ///
  /// `CairnDatabase.direct` takes the token once; `CairnDatabase.supabase`
...
/// [CairnAdapter.direct] and the push pilot's background wake, which must
/// land on the SAME SQLite file with the same scope.
Future<CairnDatabase> openCairnDirect({
  required String supabaseUrl,
  required String anonKey,
  required String accessToken,
  required String userId,
  required String dbDir,
}) => CairnDatabase.direct(
  supabaseUrl: supabaseUrl,
  anonKey: anonKey,
  // The scope the change-log trigger stamps, and the private Realtime
  // channel this device may join — see .cairn/direct.sql's
...
final CairnSchema _schema = CairnSchema(
  tables: [
    CairnTable(
      name: 'sessions',
      primaryKey: const ['id'],
    ...
    CairnTable(
      ...
      columns: [
        CairnColumn.text('id'),
        CairnColumn.text('order_id'),
        CairnColumn.text('status'),
        CairnColumn.text('previous_status'),
        CairnColumn.text('note'),
