...
**Versions:** Flutter 3.44.0 (stable) · Dart 3.12.0 · `cairn_flutter` — path dependency on `../../../sdk/cairn_flutter` (pubspec version `0.1.0`, built from cairn repo HEAD `93b980d` — no fixed pub.dev release, it tracks the workspace) · `supabase_flutter` 2.17.1

## Environment gate (checked live this session, not assumed from prior reports)

- `docker ps`: only `cairn-postgres` running (cairn's own e2e Postgres, port 5433 — unrelated to Atlet). No `atlet-services` containers (`cairn-server` on 8080) are up.
- `apps/atlet/services/.env`: absent. Only `.env.example` ships, by design — no real Supabase credentials are checked into this tree.
- `docker compose -f apps/atlet/services/docker-compose.atlet.yml config -q`: exits 0 syntactically, but every required variable (`SUPABASE_URL`, `SUPABASE_JWT_SECRET`, `CAIRN_WRITE_TABLES`) resolves blank.
...
| 2 | Row inserted via PostgREST → `remoteVisible` <60s | **Never scoped to any task.** T9's brief pinned CairnAdapter's live-run requirement to items 1, 4, 5 only (`task-9-brief.md:33`) — item 2 was never assigned to this adapter, by anyone, at any point. NOT RUN. |
...
| 4 | `signOut` wipes local DB files; re-init cold-syncs from zero | NOT RUN (live, filesystem-observed). Statically verified: `CairnAdapter.signOut()` → `CairnDatabase.signOut()` performs a full wipe per ADR-0029 — confirmed by direct code read, not by watching files disappear on disk. |
| 5 | No adapter API leaks engine types into the app/bench layer | **PASS — statically verified.** `sync_adapter.dart` (frozen, Task 7) contains no `cairn_flutter` symbol; independently confirmed by two separate reviewers across T9's and T10's verdicts. |
...
2. `docker compose -f apps/atlet/services/docker-compose.atlet.yml up -d` (`cairn-server` :8080), with `CAIRN_WRITE_TABLES=sessions`.
...
This is listed as an operator action in the "Recommended live run" section above; do not run a live DELETE-propagation check without first confirming `REPLICA IDENTITY FULL` on all three tables, or an apparent CairnAdapter defect may actually be this schema gap.
