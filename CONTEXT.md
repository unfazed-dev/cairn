...
> **Cairn:** "No — `watch()` returns a hot `ValueListenable` ref-counted per query; one re-execution fans out to all three. That's why we don't use a cold-stream-per-call — at our throughput that would storm."
>
