...
#         /tmp/cairn-remeasure-<tag>.log             progress; last line REMEASURE_EXIT=<rc>
...
LOG=/tmp/cairn-remeasure-$D.log
exec >>"$LOG" 2>&1
ulimit -n 1048576
mkdir -p "$OUT" "$RAW"
echo "START $(date '+%F %T') load=$(uptime | sed 's/.*averages: //')"
echo "BUILD start $(date '+%T')"
cargo build --release -p cairn-bench --bin cairn-bench --bin cairn-bench-10k >/tmp/cairn-bench-build.log 2>&1 \
  || { echo "BUILD FAILED (see /tmp/cairn-bench-build.log)"; echo REMEASURE_EXIT=91; exit 91; }
[ -x target/release/cairn-bench-10k ] || { echo "no target/release/cairn-bench-10k after build"; echo REMEASURE_EXIT=92; exit 92; }
...
for s in 1 2; do
  ...
  target/release/cairn-bench-10k 10000 5000 60 >"$RAW/soak-pass$s.log" 2>&1
