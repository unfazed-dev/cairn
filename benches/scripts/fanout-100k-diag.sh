...
# Bench runs are exclusive on this host: honours /tmp/cairn-bench.lock.
set -u
SRC=$1; OUT=$2
IMAGE=${CAIRN_LINUX_IMAGE:-rust:1.95-bookworm}
TAG=fanout-diag
mkdir -p "$OUT"

LOCK=/tmp/cairn-bench.lock
waited=0
...
echo $$ >"$LOCK"
trap 'rm -f "$LOCK"; docker rm -f cairn-linux-fanout-100k >/dev/null 2>&1' EXIT

docker run --rm --name cairn-linux-fanout-100k \
  -v "$SRC":/src \
  -v cairn-linux-target-fanout:/target \
  -v cairn-linux-cargo-registry-fanout:/usr/local/cargo/registry \
  ...
  -w /src "$IMAGE" sh -c '
    set -u
    TAG='"$TAG"'
    BIN=/target/bin-$TAG/cairn-bench-10k
    echo "BUILD start $(date +%T) tag=$TAG"
    cargo build --release --locked -p cairn-bench --bin cairn-bench-10k 2>&1 | tail -3 \
      || { echo BUILD_FAILED; exit 91; }
    mkdir -p "$(dirname "$BIN")" && cp /target/release/cairn-bench-10k "$BIN" || exit 92
