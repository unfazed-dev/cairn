...
# no other cairn-linux-* container is up, and /tmp/cairn-bench.lock is free.
...
NAME=cairn-linux-fanout-100k
LOG=$OUT/linux-fanout-diag.log
mkdir -p "$OUT"

host_load1() { sysctl -n vm.loadavg | awk '{print $2}'; }
other_container_up() { docker ps --format '{{.Names}}' | grep -q '^cairn-linux-'; }
...
in_container() { # args: inner-script args...
  docker run --rm --name "$NAME" \
    ...
    -e TAG=fanout-diag \
    -e CAIRN_PROBE_SKIP_DECODE="${CAIRN_PROBE_SKIP_DECODE:-}" \
