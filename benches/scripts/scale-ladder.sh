...
#
# Usage: nohup benches/scripts/scale-ladder.sh [date-tag] &   → log /tmp/cairn-ladder.log
...
cargo build --release -p cairn-bench --bin cairn-bench --bin cairn-bench-10k >/tmp/cairn-bench-build.log 2>&1 \
  || { echo "BUILD FAILED (see /tmp/cairn-bench-build.log)"; echo LADDER_EXIT=91; exit 91; }
...
for s in 1 2 3; do
  ...
  target/release/cairn-bench-10k 10000 5000 60 >"$RAW/mac-soak-idle$s.log" 2>&1
...
target/release/cairn-bench-10k 10000 5000 60 >"$RAW/mac-soak-afterbench.log" 2>&1
