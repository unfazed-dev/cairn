#!/bin/sh
# Build cairn-bench-10k inside a Linux container and run the 10k soak there.
#
...
docker run --rm --name "cairn-linux-soak-$TAG" \
  -v "$SRC":/src \
  -v cairn-linux-target:/target \
  -v cairn-linux-cargo-registry:/usr/local/cargo/registry \
