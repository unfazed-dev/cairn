...
`Hyprland`, a second `claude` session. `cairn-bench` took **636%** of 800%
...
Same host, desktop closed, `cairn-bench` alone on the machine. Load average
...
`crates/cairn-bench/src/main.rs` blames the 10k regime on `FanOutService::run`
...
about Cairn's server limit: this is still the in-process harness with the
...
A real deployment scoped by tenant (`CAIRN_TENANT_COLUMN`) fans each row to one
