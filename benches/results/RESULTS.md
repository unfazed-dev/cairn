# Cairn Week-1 Benchmark — Results

> Generated by `cairn-bench`. Methodology: [`docs/BENCHMARK-METHODOLOGY.md`](../docs/BENCHMARK-METHODOLOGY.md).
...
PowerSync publishes no comparable aggregate fan-out figure. Its published rates are 2,000–4,000 ops/sec **replication ingest** (Postgres → PowerSync Service, a different pipeline stage) and 2,000–20,000 ops/sec **per-client sync** (PowerSync Service → Client, not aggregate). Cairn's measurement below is an aggregate fan-out rate (fanning row-change events to connected clients) with a synthetic replicator on loopback — see [Correction](#correction-2026-08-06).
...
- **Op-log write path (ADR-0025 slice 2):** this number is the **fan-out
  ...
  (`CAIRN_BENCH_OPLOG=1`, which mirrors `PgOpLogWriter`'s `append` = `try_send`
  ...
  "pending"): `crates/cairn-infra/tests/e2e_pg_write_amp.rs` wires the production
...
push doorbell path attached (`CAIRN_BENCH_PUSH=1` — bounded hint channel +
`CountingNotifier` consumer, mirroring the op-log toggle methodology above):

| Run | ops/sec | drop% | delivered |
|---|---|---|---|
| pre-push binary (`target/tmp/cairn-bench-before`) | 833,319 | 0.00% | 99,998,946 |
...
Same-stage pair re-run with the cairn-pushd daemon + RemoteNotifier delegation
...
| ON (`CAIRN_BENCH_PUSH=1`) | 833,304 | 0.00% | 99,997,299 |
...
DOWN (cairn-infra `push/remote.rs` tests); a loopback daemon would measure
...
`crates/cairn-bench/src/bin/bench_pg_ingest.rs` (`--features pg`). **STAGE
...
checkpoints, acks. Harness: `crates/cairn-client/examples/apply_bench.rs`.
...
`crates/cairn-client/tests/e2e_pg_apply_throughput.rs` (`--features pg`,
...
`cairn-bench --clients 1000 --events 100000`, profile `small`, buffer 1024 —
three passes back-to-back, then two `cairn-bench-10k 10000 5000 60` soaks.
...
Why the figure is so stable: `cairn-bench` runs a fixed 120 s window
...
### 10k-client soak (`cairn-bench-10k 10000 5000 60`) — measured, NOT citable as throughput
...
to hit `ENOBUFS` at ~9.2k loopback sockets (mbuf-cluster exhaustion, not cairn
...
`cairn-bench-10k 10000 5000 60`, baseline = `e33b4c3` (sequential loop, per-session
...
**macOS 10k soak (`cairn-bench-10k 10000 5000 60`, two back-to-back runs):**
...
**Tier shape.** `cairn-bench-10k <clients> 5000 <window> 1 <listeners>` — 5,000
...
## The fan-out walk is linear — MEASURED 2026-09-21 (macOS 10-core, `cairn-fanout-walk`)

`crates/cairn-bench/src/bin/fanout_walk.rs` runs the real `InMemorySessionStore`,
...
runtime (`cairn-bench-10k` runs the swarm in-process). The next experiment is
...
`cairn-fanout-walk` with the new `sink=wire` mode — the full server path
...
isolated `cairn-fanout-walk` numbers. **That conclusion was too strong and is
...
`ack_progress_every` events. `cairn-server` shipped that at **1** — a per-event
...
**Shipped:** `CAIRN_ACK_PROGRESS_INTERVAL` default 1 → **16** — the knee of the
...
`cairn-fanout-walk` has no clients, so `record_ack` is never called: the fold
