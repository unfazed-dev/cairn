...
/// [`CairnEngine`] / [`CairnSocket`] work with either backend without generics
...
impl Storage for WebStorage {
    ...
    fn horizon(&self) -> cairn_core::Result<Option<String>> {
        match self {
            WebStorage::Memory(s) => s.horizon(),
            WebStorage::SqliteWasm(s) => s.horizon(),
        }
    }
    fn save_horizon(&mut self, horizon: &str) -> cairn_core::Result<()> {
        match self {
...
///
/// The suite lives in `cairn_core::conformance` because a property proved on
...
/// `wasm-pack build crates/cairn-ffi-wasm --target web --features conformance`.
#[cfg(feature = "conformance")]
#[wasm_bindgen(js_name = cairnConformance)]
pub fn cairn_conformance(db: Option<js_sys::Object>) -> js_sys::Array {
    ...
    cairn_core::conformance::run_all(|| match db.as_ref() {
...
impl WebStorage {
    ...
    pub(crate) fn read_payload(
        ...
        table: &str,
        pk: &str,
    ) -> cairn_core::Result<Option<Vec<u8>>> {
    ...
    pub(crate) fn query_json(&self, sql: &str) -> cairn_core::Result<String> {
        match self {
            WebStorage::Memory(_) => Ok("[]".to_string()),
            WebStorage::SqliteWasm(s) => s
                .query_json(sql)
                .map_err(|e| cairn_core::StorageError::Backend(e.to_string())),
        }
    }

    /// Materialize WS2 read-views. SqliteWasm only (Memory has no views).
    /// No-op on Memory (rows are already accessible by table).
    pub(crate) fn apply_schema(&self, tables: &[(String, Vec<String>)]) -> cairn_core::Result<()> {
        match self {
...
pub struct CairnEngine {
    ...
    hlc_state: Cell<Option<cairn_domain::Hlc>>,
}
...
impl CairnEngine {
    ...
    /// the Worker (via [`CairnSocket::connect`]'s `db_handle` arg) after async
    /// sqlite-wasm init. The JS `db` is a wrapper around the sqlite-wasm
    /// instance (see `sdk/cairn_web/worker/sqlite_wasm_glue.js`). Browser-only.
    #[doc(hidden)]
    ...
    /// storage directly; only [`CairnSocket`]'s write/flush path (WS1) does.
    ...
    // `cairn-domain` (reused, NOT re-implemented). The native `SyncClient` is
    ...
    fn or_set_op(
        ...
        let h = self.mint_hlc();
        let element_struct = cairn_domain::OrSetElement {
            v: element.to_string(),
    ...
    fn counter_op(&mut self, table: &str, pk: &str, delta: i64) -> Result<f64, JsValue> {
        ...
        let existing = s
            ...
            .unwrap_or_default();
        let payload_bytes = cairn_domain::counter_apply_delta(&existing, &self.replica_id, delta);
    ...
    fn mint_hlc(&self) -> cairn_domain::Hlc {
        ...
        let h = cairn_domain::Hlc::mint(prev, now_wall_ms);
...
impl CairnSocket {
    ...
    pub fn write(
        ...
        let op_enum = WriteOp::from_wire_str(op).ok_or_else(|| {
            JsValue::from_str("cairn write: invalid op (expected upsert|delete|patch)")
        })?;
        ...
        let id = engine
            .storage_mut()
            .enqueue(write.clone())
            .map_err(|e| JsValue::from_str(&format!("cairn write: enqueue: {e}")))?;
    ...
    pub async fn resume(&self) -> Result<bool, JsValue> {
        ...
        Err(JsValue::from_str(
            "resume: socket is closed — call CairnSocket.connect() with the \
    ...
    // transactional `writeBatch` lived ONLY on the in-process `CairnEngine`
    // (4a), but the Flutter-web Worker drives `CairnSocket` (the transport
    ...
    // reuse `CairnEngine`'s logic verbatim (HLC mint, `cairn-domain` CRDT
    // algebra, `enqueue_batch` atomicity) — no CRDT algebra is re-implemented
    // here, and `CairnEngine`/native are untouched. The only addition over a
...
mod tests {
    ...
    fn clear_wipes_in_memory_rows() {
        // ADR-0029 D1: the sign-out wipe at the CairnEngine seam. The wipe
        // semantics are unit-tested in cairn-core; this proves the seam calls
...
mod transport_tests {
    ...
    fn pump_committed_true_when_only_trailing_flush_committed() {
        ...
        // common browser path (one Cairn event per WS message → flushed at
        // message end).
        let pump = PumpResult {
            applied: 1,
            ack: None,
...
mod typed_verb_tests {
    ...
    fn set_crdt_tables_enables_counter_merge() {
        ...
        let value = cairn_domain::counter_value(&payload.unwrap()).unwrap();
    ...
    fn or_set_add_renders_element_locally() {
        ...
        let present = cairn_domain::present_elements(&payload).unwrap();
        assert!(
    ...
    fn or_set_remove_tombstones_element() {
        ...
        let payload = eng.read_payload("tags", "row1").unwrap().unwrap();
        assert!(
            cairn_domain::present_elements(&payload)
                .unwrap()
    ...
    fn counter_increment_accumulates_same_replica() {
        ...
        let v1 = cairn_domain::counter_value(&payload).unwrap();
        ...
        let payload = eng.read_payload("likes", "post1").unwrap().unwrap();
        let v2 = cairn_domain::counter_value(&payload).unwrap();
    ...
    fn counter_decrement_subtracts_value() {
        ...
        let v = cairn_domain::counter_value(&payload).unwrap();
    ...
    fn counter_increment_multiple_replicas_merge() {
        ...
        let merged = cairn_domain::merge_counter_or_lww(&payload1, &payload2);
        let total = cairn_domain::counter_value(&merged).unwrap();
    ...
    fn replica_id_is_unique_per_engine() {
        let eng1 = CairnEngine::new();
        let eng2 = CairnEngine::new();
        assert_ne!(
