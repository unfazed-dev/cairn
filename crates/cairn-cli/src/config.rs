...
pub fn config_path(dir: &Path) -> PathBuf {
    cairn_infra::config_path::resolve(dir, DEFAULT_FILE_NAME, LEGACY_FILE_NAME)
}
...
/// Pre-rename [`DOT_CAIRN_DIR`], read and written in place until 1.0.
pub const LEGACY_DOT_DIR: &str = ".cairn"; // rename:hold — pre-rename config dir, read as fallback until 1.0 (decision 10, ADR-0046)

/// The tool dir in `dir`: [`DOT_CAIRN_DIR`], or the pre-rename dir when only
/// that exists (ADR-0046).
#[must_use]
pub fn dot_dir(dir: &Path) -> PathBuf {
    cairn_infra::config_path::resolve(dir, DOT_CAIRN_DIR, LEGACY_DOT_DIR)
}
...
/// the historical shape: a cairn-server holds the replication slot and fans
...
pub enum LinkMode {
    /// Sync through a cairn-server WebSocket. The default, so an existing
    /// `config.json` written before direct mode existed still parses.
    #[default]
    Server,
    /// Sync straight from Postgres. No Cairn server in the path.
    Direct,
}
