...
use cairn_infra::rules_file::{self, RULES_FILE_NAME};

use crate::config::{Backend, LinkMode, ProjectConfig, DOT_CAIRN_DIR, LOCAL_DIR};
use crate::direct;
use crate::prompt::prompt_nonempty;

#[derive(Debug, Args)]
pub struct LinkArgs {
    /// `server` (a cairn-server holds the slot) or `direct` (the device syncs
    /// straight from the client's own Postgres — no Cairn server at all).
    ...
    /// Repeat per table. Server mode's `CAIRN_PUSH_TABLES` grammar:
    ...
    /// (`supabase login` first) — apply `.cairn/direct.sql` with `pg_net`, mint
...
/// socket the doorbell joins. Everything else in `.cairn/config.json` is
/// unchanged, and the generated SQL lands in `.cairn/direct.sql`.
fn run_direct(args: LinkArgs, cwd: &Path) -> Result<()> {
    if args.sync_url.is_some() {
        bail!(
            "--sync-url is server mode only: direct mode has no cairn-server to \
    ...
    let Backend::Supabase { url, .. } = &backend else {
        bail!(
            "direct mode requires `--backend supabase`: the generated SQL calls \
             ...
             none of which exist on a plain Postgres. Run a cairn-server for those \
             (`--mode server`)."
        );
    };
    ...
    let rules = rules_file::load(&rules_path)?.ok_or_else(|| {
        anyhow::anyhow!(
            "no {RULES_FILE_NAME} at {} — direct mode generates one trigger per \
             synced table, so it needs the table list first: run `cairn rules init`",
            rules_path.display()
        )
    })?;
    ...
    let sql_path = cwd.join(DOT_CAIRN_DIR).join(direct::OUTPUT_FILE);
    ...
    println!("\u{2713} wrote `.cairn/config.json` (mode: direct)");
    println!(
        "\u{2713} wrote `.cairn/{}` \u{2014} {} table(s), retention {}",
        direct::OUTPUT_FILE,
        tables.len(),
        args.retention
    );
    ...
    if let (true, Some(service_account)) = (args.deploy, &args.fcm_service_account) {
        ...
        println!(
            "\u{2713} applied `.cairn/{}`, set the push secret, deployed cairn-push",
            direct::OUTPUT_FILE
        );
        println!("next: turn OFF \"Allow public access\" in the project's Realtime settings,");
    } else {
        println!(
            "next: apply it \u{2014} `psql \"$DATABASE_URL\" -f .cairn/{}`",
            direct::OUTPUT_FILE
        );
    ...
    println!("      then `cairn doctor --mode direct` to check it landed.");
    Ok(())
}
...
/// gitignored `.cairn/local/`, never in argv (visible to `ps`), and the files
...
fn deploy(
    ...
    let local = cwd.join(DOT_CAIRN_DIR).join(LOCAL_DIR);
    let sql_file = local.join("deploy.sql");
    let env_file = local.join("cairn-push.env");
    let result = (|| {
        write_private(
            &sql_file,
            &format!(
        ...
        write_private(
            &env_file,
            &format!("CAIRN_PUSH_SECRET={secret}\nFCM_SERVICE_ACCOUNT='{sa_line}'\n"),
        )?;
...
/// the endpoint `cairn_client::doorbell` joins.
...
mod tests {
    ...
    fn app_dir() -> PathBuf {
        let dir =
            std::env::temp_dir().join(format!("cairn-link-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(dir.join(DOT_CAIRN_DIR).join(LOCAL_DIR)).unwrap();
        std::fs::write(
            dir.join("sa.json"),
    ...
    fn deploy_sets_one_secret_on_both_sides_and_leaves_no_secret_behind() {
        ...
        assert!(log.contains(&format!("CAIRN_PUSH_SECRET={secret}\n")));
        ...
        let local = dir.join(DOT_CAIRN_DIR).join(LOCAL_DIR);
        assert_eq!(
