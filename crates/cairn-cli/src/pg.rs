...
impl PgControl {
    ...
    /// The underlying control-plane connection. `cairn doctor --mode direct`
    ...
    /// by an *abandoned* slot — cairn-server's client eviction can't help once
...
/// verification — which keeps `docker compose up` + `cairn init` working with
...
mod tests {
    ...
    /// `cairn doctor --mode direct` unusable against Supabase.
    #[test]
