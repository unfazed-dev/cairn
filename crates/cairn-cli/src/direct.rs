...
//!
//! Direct mode has no Cairn server: the device pulls from the client's own
//! Postgres through PostgREST and is woken by a Realtime broadcast. Everything
//! that makes that safe lives in the database, so `cairn link --mode direct`
...
//!
//! ## Why `cairn_pull` lives in `public`, not in `cairn`
//!
...
//! entry points in `public` under a `cairn_` prefix removes both: the client
...
use cairn_domain::{ScopeExpr, ScopeOp, ScopeValue, SyncMode, SyncRules};

/// The primary-key column direct mode assumes on every synced table. Matches
/// `cairn_client::postgrest`'s `PK_COLUMN` and `PendingWrite`'s v1 convention.
...
/// `.cairn/direct.sql` — the generated file, relative to `.cairn/`.
...
/// make every push attempt a change row, which fires another push. `cairn`'s
...
pub struct PushConfig {
    ...
    pub cooldown: String,
    /// `cairn link --visible`: the tables whose changes arrive as the
...
/// `CAIRN_PUSH_TABLES` visible/action entries, so one line of config moves
...
///
/// ponytail: a second parser of the grammar in cairn-server's
/// `parse_push_tables`; move both into cairn-infra when a third caller shows.
///
/// # Errors
...
/// Turn `cairn_rules.toml` into a direct-mode table plan, refusing anything
...
pub fn plan(rules: &SyncRules, public_tables: &[String]) -> Result<Vec<DirectTable>> {
    ...
    if active.is_empty() {
        bail!(
            "no synced tables in cairn_rules.toml (sync_mode = {}) — run \
             `cairn rules init` first; direct mode needs an explicit table list to \
    ...
    if let Some(stray) = public.iter().find(|t| !synced.contains(*t)) {
        bail!("`--public {stray}` names a table that is not synced in cairn_rules.toml");
    }
...
fn header(s: &mut String, tables: &[DirectTable]) {
    let _ = writeln!(
        s,
        "-- Generated by `cairn link --mode direct`. Re-runnable: every object is\n\
         -- created with `if not exists` or dropped first, so applying it twice is a\n\
         -- no-op. Regenerate after editing cairn_rules.toml and diff the two files.\n\
         --\n\
         -- Protocol: docs/plans/direct-mode-sync-protocol.md\n\
         -- Client:   cairn_core::pull (cursor) + cairn_client::postgrest (transport)\n\
         --\n\
         -- Synced tables:"
    );
    for t in tables {
...
-- settings. `cairn doctor --mode direct` checks that; SQL cannot.
...
-- with pg_cron:
--   select cron.schedule('cairn-prune', '0 * * * *', $$select cairn.prune()$$);
...
fn push_path(s: &mut String, cfg: &PushConfig) {
    ...
    let _ = write!(
        ...
        r#"
-- ===========================================================================
-- Push (`cairn link --mode direct --push <url>`). A killed app that never
...
-- mode's CAIRN_PUSH_TABLES). iOS never wakes a user-quit app for a silent
...
-- non-null category makes it an action push. The rows come from `cairn link
...
-- rather than a select on cairn.push_tokens. `cairn` is deliberately not an
...
begin
  ...
  -- pg_net is an extension and may not be installed; `cairn doctor --mode
...
// ---------------------------------------------------------------------------
// `cairn doctor --mode direct` — read-only verification of a deployed schema.
// ---------------------------------------------------------------------------
...
/// One line of `cairn doctor --mode direct` output.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct Check {
    pub verdict: Verdict,
    pub label: String,
...
pub async fn inspect(
    ...
    if !log_exists {
        out.push(Check::note(
            "nothing else can be checked \u{2014} run `cairn link --mode direct` \
             and apply .cairn/direct.sql",
        ));
        return Ok(out);
    }
                ...
                if scalar {
                    ...
                    format!(
                        "{proname} returns a SET \u{2014} PostgREST truncates it at \
                         ...
                         Regenerate with `cairn link --mode direct`."
                    )
                },
            ));
        }
    ...
    // in the scope. Neither is reachable through `cairn link` — it refuses the
    ...
    out.push(Check::new(
        ...
        if feedback.is_empty() {
            "neither pg_net nor cairn's own tables are instrumented".to_string()
        } else {
            format!(
    ...
    if push_installed {
                ...
                out.push(Check::new(
                    ...
                    format!(
                        "anon may execute {signature} \u{2014} it is security definer, so \
                         this is a hole, not defence in depth. Re-run `cairn link --mode \
        ...
        // `cairn` is not an exposed schema. A function that reads the table
        ...
        out.push(Check::new(
            ...
            if targets_rpc {
                ...
                "public.cairn_push_targets is MISSING \u{2014} the Edge Function would have \
                 ...
                 cairn` and drops every notification. Re-run `cairn link --mode direct \
                 --push`."
                    .to_string()
            },
        ));
...
mod tests {
    use super::*;
    use cairn_domain::{HandRule, StreamRule, TableRule, RULES_VERSION};
    ...
    fn streams_and_an_empty_table_set_are_refused() {
        ...
        assert!(err(&none, &[]).contains("cairn rules init"));
    }
    ...
    /// Pins the contract `cairn_core::pull` and `cairn_client::postgrest`
    ...
    fn the_generated_sql_matches_the_client_contract() {
        ...
        for needle in [
            ...
            "where c.xid >= since and c.xid < h.horizon",
            "limit greatest(max_txns, 2)",
            // The topic `cairn_client::doorbell` joins.
    ...
    fn push_is_opt_in_and_never_instruments_its_own_tables() {
        ...
        for needle in [
            ...
            // The Edge Function's only way into the registry: `cairn` is not an
        ...
        // The trigger is attached to public tables only — never to cairn's own.
    ...
    #[test]
    fn visible_specs_parse_like_cairn_push_tables_and_render_quoted() {
    ...
    fn rendering_is_deterministic_and_re_runnable() {
        ...
        let sql = sample_sql();
        assert!(sql.starts_with("-- Generated by `cairn link --mode direct`"));
