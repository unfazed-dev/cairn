//! Real-Postgres e2e for the SQL `cairn link --mode direct` generates.
//!
//! Env-gated exactly like `e2e_pg_cli.rs` — skipped unless `CAIRN_E2E_PG=1`:
//!
//! ```sh
//! make pg-up
//! CAIRN_E2E_PG=1 cargo test -p cairn-cli --test e2e_pg_direct_sql -- --test-threads=1
//! ```
//!
//! ## What this is for
//!
//! `crates/cairn-cli/src/direct.rs` has unit tests, but they only prove the
...
//! database that has a real `auth.users`, so pointing `CAIRN_PG_URL` at a live
//! Supabase project aborts instead of dropping its auth schema.

use cairn_cli::direct::{
...
do $$ begin
  ...
  -- requirement, and `cairn link --mode direct` already refuses any backend
  -- but Supabase.
...
async fn assert_disposable(client: &tokio_postgres::Client) {
    ...
    assert!(
        !real_supabase,
        "CAIRN_PG_URL points at a database with a real `auth.users` — refusing to run: \
         this test drops the `cairn`, `auth` and `realtime` stub schemas"
    );
}
...
impl Fixture {
    async fn setup() -> Self {
        ...
        let tasks = format!("cairn_direct_{}", uuid::Uuid::new_v4().simple());
        ...
        let sql = render_with_push(
            ...
            DEFAULT_RETENTION,
            Some(&PushConfig {
                endpoint: "https://example.test/cairn-push".to_string(),
        ...
        // `cairn doctor --mode direct` fails while it is unset, on purpose.
        client
...
async fn an_in_flight_transaction_hides_every_later_commit() {
    if cairn_infra::env::var(E2E_FLAG).ok().as_deref() != Some("1") {
...
/// `cairn doctor --mode direct` has to pass a fresh deploy, and — the reason
...
async fn a_visible_template_pushes_every_change_with_its_row() {
    ...
    let fx = Fixture::setup().await;
    // Exactly what `cairn link --visible` writes, so its quoting meets a real
    // Postgres here.
    let spec = format!(
