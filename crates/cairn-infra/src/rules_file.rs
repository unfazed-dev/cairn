//! `cairn_rules.toml` load/save (ADR-0031).
//!
...
//! ([`RulesFileMirror`]) maps the TOML tables to [`cairn_domain::SyncRules`],
//! keeping the domain type itself free of TOML-specific `serde` derives
//! (`cairn-domain` stays I/O-free — see `CLAUDE.md`).
//!
...
use cairn_domain::{HandRule, RulesError, SyncMode, SyncRules, TableRule};
use serde::{Deserialize, Serialize};

/// Default file name, resolved relative to the process working directory.
pub const RULES_FILE_NAME: &str = "cairn_rules.toml";
/// Pre-rename file name, read (and edited in place) as a fallback until 1.0.
pub const LEGACY_RULES_FILE_NAME: &str = "cairn_rules.toml"; // rename:hold — pre-rename config name, read as fallback until 1.0 (decision 10, ADR-0046)

/// The rules file in `dir` — [`RULES_FILE_NAME`], or the pre-rename name when
/// only that exists (ADR-0046). For readers and writers alike.
#[must_use]
pub fn path_in(dir: &Path) -> PathBuf {
    crate::config_path::resolve(dir, RULES_FILE_NAME, LEGACY_RULES_FILE_NAME)
}

const BANNER: &str = "# cairn_rules.toml — generated by `cairn rules init` (ADR-0031).\n\
...
/// The serde-facing mirror of `cairn_rules.toml`. Kept private and separate
...
impl RulesFileMirror {
    ...
    fn into_domain(self, path: &Path) -> Result<SyncRules, RulesFileError> {
        ...
        let streams = self
            .streams
            .into_iter()
            .map(|(name, entry)| cairn_domain::StreamRule {
                name,
                table: entry.table,
...
mod tests {
    use super::*;
    use cairn_domain::RULES_VERSION;

    #[test]
    fn legacy_rules_file_name_is_the_pre_rename_name() {
        assert_eq!(LEGACY_RULES_FILE_NAME, "cairn_rules.toml"); // rename:hold — pins the fallback to the file deployments already have
    }
    ...
    fn temp_rules_path() -> std::path::PathBuf {
        let dir =
            std::env::temp_dir().join(format!("cairn-rules-file-test-{}", uuid::Uuid::new_v4()));
    ...
    fn streams_edit_changes_the_checksum_under_every_mode() {
        ...
        for mode in [SyncMode::All, SyncMode::Toggles, SyncMode::Hand] {
            ...
            with_stream.streams = vec![cairn_domain::StreamRule {
                name: "lists".to_string(),
                table: "lists".to_string(),
