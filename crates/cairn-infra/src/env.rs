...
//!
//! Every `CAIRN_*` read in the workspace goes through [`var`] / [`var_os`]
...
/// `tracing`: clap resolves env before the subscriber exists (`CAIRN_LOG` is
...
/// plain `map.get("CAIRN_…")` lookups get the fallback.
...
/// Point each `env = "CAIRN_*"` arg at its legacy var when only that one is
