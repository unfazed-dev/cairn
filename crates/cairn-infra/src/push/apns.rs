...
impl ApnsRail {
        ...
        let pem = if p8.contains("-----BEGIN") {
            ...
            std::fs::read_to_string(&p8).map_err(|e| {
                ...
                PushRailError(format!(
                    "CAIRN_APNS_KEY_P8: not a PEM (no -----BEGIN) and not a \
                     readable path (value is {} chars): {e}",
                    p8.len()
                ))
            })?
        };
        let sandbox = crate::env::var("CAIRN_APNS_SANDBOX").is_ok_and(|v| v == "1");
    ...
    ///   data ages out) is deliberately NOT set: cairn cannot know when the
...
mod tests {
    ...
    async fn apns_liveactivity_headers_and_payload() {
        ...
        assert_eq!(
            ...
            Some("dev.cairn.app.push-type.liveactivity")
        );
    ...
    async fn e2e_apns_smoke() {
        if crate::env::var("CAIRN_E2E_APNS").is_err() {
            eprintln!(
        ...
        let token = crate::env::var("CAIRN_E2E_APNS_TOKEN").expect("CAIRN_E2E_APNS_TOKEN");
        let outcome = rail
            .send(
                &token,
                Some("cairn-e2e"),
