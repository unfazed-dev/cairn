...
pub enum PushPayload {
    ...
    Visible {
        ...
        ///
        /// Still not a data channel. Apple's guidance is the rule cairn
...
/// - `title`, `body`, `category`, `table`, `lsn` — cairn's own: the FCM
...
/// it: the pushd HTTP API (`cairn-push`) and the server's `CAIRN_PUSH_TABLES`
...
pub fn validate_data(data: &PushData) -> Result<(), String> {
    for key in data.keys() {
        ...
        if reserved_data_key(key) {
            return Err(format!(
                "payload data key {key:?} is reserved by APNs/FCM or by cairn"
            ));
        }
    }
