...
//!    (`CAIRN_PUSH_REMOTE_STATE_PATH` — see [`CursorFile`]).
//!
...
impl RemoteNotifier {
    ...
    pub fn new(
        ...
        if let Some(path) = &receipts_state_path {
            ...
            info!(
                "remote push receipts cursor is in-memory (CAIRN_PUSH_REMOTE_STATE_PATH \
...
struct VisibleBody {
    ...
    /// running an older cairn-pushd still accepts every push that carries no
    /// routing keys.
...
async fn receipt_loop(
    ...
    // (CAIRN_PUSH_REMOTE_STATE_PATH): loaded once here, written back after
...
mod tests {
    ...
    fn send_request_shape_platform_collapse_key_metadata() {
        ...
        assert_eq!(
            ...
            "None category omitted, not serialized as null; empty data likewise — \
             an older cairn-pushd must still accept this body"
        );
    ...
    fn temp_state_path(label: &str) -> PathBuf {
        ...
        std::env::temp_dir().join(format!(
            "cairn-remote-cursor-{}-{label}-{n}.json",
            std::process::id()
        ))
    }
