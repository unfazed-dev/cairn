...
//! chart drawing the Cairn throughput curve against PowerSync's published
//! ceiling.
...
/// to [`UNKNOWN`] on any error. The legacy `CAIRN_BENCH_HOST` env override is
/// still honored (useful for reproducible local runs).
pub fn hostname() -> String {
    if let Ok(h) = std::env::var("CAIRN_BENCH_HOST") {
        return h;
    }
...
pub fn write_reports(cfg: &BenchConfig, runs: &[RunResult], env: &Environment) -> Result<()> {
    ...
    fs::write(
        Path::new(&cfg.out_dir).join("cairn-bench-results.json"),
        json,
    )
    .context("write json")?;
...
fn render_markdown(r: &FullReport) -> String {
    ...
    s.push_str("# Cairn Week-1 Benchmark — Results\n\n");
    s.push_str("> Generated by `cairn-bench`. Methodology: ");
    ...
    s.push_str(
        "PowerSync publishes a **server-side ceiling of ~2,000–4,000 ops/sec** for small rows. \
         Cairn's measurement is of the same logical operation (fanning row-change events to \
...
fn render_svg(r: &FullReport) -> String {
    ...
    svg.push_str(&format!(
        "<text x=\"{}\" y=\"20\" font-size=\"16\" font-weight=\"bold\">Cairn throughput vs PowerSync ceiling (ops/sec)</text>\n",
        pad_l
    ));
