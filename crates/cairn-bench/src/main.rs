...
async fn main() -> Result<()> {
    init_tracing();
    let cfg = cairn_infra::env::parse::<BenchConfig>();
...
async fn run_one(cfg: &BenchConfig, clients: usize, rep: usize) -> Result<RunResult> {
    ...
    // mirroring PgOpLogWriter's hot path). Set CAIRN_BENCH_OPLOG=1 for the
    ...
    let op_log: Option<Arc<cairn_infra::RecordingOpLogWriter>> =
        if cairn_infra::env::var_os("CAIRN_BENCH_OPLOG").is_some() {
            Some(Arc::new(cairn_infra::RecordingOpLogWriter::new(4096)))
        } else {
            None
        };
    ...
    // CAIRN_BENCH_OPLOG above. The counting consumer (a NoopNotifier plus an
    ...
    let push_count: Option<Arc<AtomicU64>> =
        cairn_infra::env::var_os("CAIRN_BENCH_PUSH").map(|_| Arc::new(AtomicU64::new(0)));
    let fanout = Arc::new({
...
/// The CAIRN_BENCH_PUSH counting rail: a no-op forward that counts hints, so
...
impl cairn_application::ports::PushNotifier for CountingNotifier {
    async fn notify(&self, _hint: cairn_application::ports::PushHint) {
...
async fn client_task(
    ...
    while let Some(Ok(msg)) = read.next().await {
        ...
        let n = cairn_infra::wire::decode_frames(&bytes).len() as u64;
        if n > 0 {
