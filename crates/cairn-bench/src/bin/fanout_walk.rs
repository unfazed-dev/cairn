//! `cairn-fanout-walk` — isolates the per-event fan-out walk. No sockets, no
...
//!
//! Usage: `cairn-fanout-walk <clients> <events> [sink=tokio|noop|wire] [buffer] [workers]`
//!
...
use cairn_domain::{ColumnValue, Lsn, Predicate, ReplicationEvent, RowOp, SyncSession};
use cairn_infra::router::SinkMsg;
use cairn_infra::store::InMemorySessionStore;
use cairn_infra::TokioEventSink;
                ...
                while let Some(msg) = rx.recv().await {
                    if encode {
                        if let SinkMsg::Event(e) = &msg {
                            std::hint::black_box(cairn_infra::wire::encode_event(e));
                        }
                    }
