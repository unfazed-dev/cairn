...
/// `cairn-fanout-walk`, 2026-09-21), so splitting a small matched set is pure
...
pub struct FanOutService {
    ...
    /// default disabled ([`EvictionPolicy::disabled`]); cairn-server enables it
...
impl FanOutService {
    ...
    /// root (`CAIRN_PUSH_TABLES`); this layer never reads env.
    ///
        ...
        if let Some(tx) = &self.push {
            ...
            if template.is_some() {
                ...
                if let Some(col) = &self.push_tables.tenant_column {
                    ...
                    // CAIRN_TENANT_COLUMN here at a non-stamped column makes
...
mod tests {
    ...
    use bytes::Bytes;
    use cairn_domain::{Lsn, Predicate, Principal, RowOp, SessionId, SyncSession};
    ...
    async fn superseded_is_counted_apart_from_both_delivered_and_dropped() {
        ...
        // fewer frame reaches the wire, and cairn-bench infers its drop rate
