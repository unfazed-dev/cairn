...
/// cairn-infra's transport). tower-http's `DefaultMakeSpan` records the whole
...
/// class; it stops Cairn from being the one that leaks it.
...
pub struct Config {
    ...
    ///
    /// `CAIRN_SYNC_AUTH=none` injects no tenant filter, and `CAIRN_BIND`
    ...
    #[arg(long, env = "CAIRN_RESYNC_SIGNAL", default_value_t = false)]
    resync_signal: bool,
    ...
    #[arg(long, env = "CAIRN_JWT_ISSUERS", default_value = "")]
    jwt_issuers: String,
    ...
    #[arg(long, env = "CAIRN_JWKS_MAX_STALE_SECS", default_value_t = 1800)]
    ...
    #[arg(
        long,
        env = "CAIRN_ALLOW_JWT_WITHOUT_EXP",
        default_value_t = false,
        value_parser = parse_env_bool
    )]
    ...
    /// that fetch `/schema` without a token — `cairn pull`, and any Flutter app
    ...
    #[arg(
        long,
        env = "CAIRN_PER_PRINCIPAL_SESSION_CAP",
        default_value_t = cairn_application::session::DEFAULT_PER_PRINCIPAL_SESSION_CAP
    )]
    per_principal_session_cap: u64,

    #[arg(
        long,
        env = "CAIRN_SNAPSHOT_MAX_ROWS",
        default_value_t = cairn_application::ports::DEFAULT_SNAPSHOT_MAX_ROWS
    )]
    ...
    #[arg(long, env = "CAIRN_RULES_FILE", default_value = cairn_infra::rules_file::RULES_FILE_NAME)]
    rules_file: String,
    ...
    /// `coalesced_ack_progress_lags_but_never_overshoots` test in cairn-application.
    ///
    ...
    /// The shared bearer secret for `CAIRN_SYNC_AUTH=bearer`: every /sync
    ...
    /// Required (non-empty) when `CAIRN_SYNC_AUTH=bearer`; ignored otherwise.
    #[arg(long, env = "CAIRN_SYNC_BEARER_TOKEN", default_value = "")]
    ...
    /// CAIRN_BIND) or "iroh" (serve sync sessions natively over an iroh
    /// endpoint, printing the QR-native iroh:// dial URL; the HTTP ops
    /// surface still binds CAIRN_BIND). Requires a build
    /// with `--features iroh`.
    #[arg(long, env = "CAIRN_TRANSPORT", default_value = "ws")]
    transport: String,
    ...
    /// gets on upgrade.
    ///
    /// Separate from `CAIRN_CORS_ORIGINS` on purpose: CORS governs the REST
    ...
    #[arg(long, env = "CAIRN_WS_ORIGINS", default_value = "")]
    ws_origins: String,
    ...
    #[arg(long, env = "CAIRN_SLOT_MAX_LAG", default_value_t = 1_073_741_824)]
    slot_max_lag: u64,
...
async fn main() -> anyhow::Result<()> {
    ...
    if cfg.rules_file == cairn_infra::rules_file::RULES_FILE_NAME {
        let resolved = cairn_infra::rules_file::path_in(std::path::Path::new(""));
    ...
    let license_secret = cairn_infra::env::var("CAIRN_LICENSE_SECRET").unwrap_or_default();
    let entitlement =
    ...
    let auth: Arc<dyn cairn_application::ports::SyncAuth> = match cfg.sync_auth.as_str() {
        "supabase-jwt" => {
            ...
            Arc::new(
                cairn_infra::SupabaseJwtAuth::from_config(secret, jwks_url)
        ...
        "bearer" => {
            if cfg.sync_bearer_token.is_empty() {
                anyhow::bail!(
                    "CAIRN_SYNC_AUTH=bearer requires CAIRN_SYNC_BEARER_TOKEN \
            ...
            Arc::new(cairn_infra::StaticBearerAuth::new(&cfg.sync_bearer_token))
        }
        "none" => {
            ...
            if !cfg.insecure_anonymous && exposes_anonymous_sync(&cfg.bind) {
                anyhow::bail!(
                    "refusing to start: CAIRN_SYNC_AUTH=none (no authentication, no \
                     tenant filter) with CAIRN_BIND={} (reachable off-host). Pick one: \
                     set CAIRN_SYNC_AUTH=supabase-jwt (or bearer) for a real deploy; \
                     set CAIRN_BIND=127.0.0.1:8800 to keep it local; or set \
                     CAIRN_INSECURE_ANONYMOUS=1 if something in front of this server \
        ...
        other => anyhow::bail!(
            "unknown CAIRN_SYNC_AUTH value: {other} (expected 'none', 'bearer', or 'supabase-jwt')"
        ),
    };
    ...
    // (finding 1, ADR-0043). `CAIRN_SLOT_MAX_LAG=0` opts back OUT, which
    ...
    if eviction.max_lag.is_none() {
        warn!(
            "CAIRN_SLOT_MAX_LAG=0: WAL-bloat eviction is OFF — a client that never acks pins the \
             replication slot and grows WAL on the primary without bound (disk exhaustion). \
             Set CAIRN_SLOT_MAX_LAG (default 1073741824 = 1 GiB) and Postgres \
             max_slot_wal_keep_size / CAIRN_PG_SLOT_WAL_KEEP_SIZE (ADR-0043)."
        );
    }
    ...
    match cfg.replicator.as_str() {
        ...
        "mirror" => {
            ...
            if !cfg.pg_url.trim().is_empty() {
                anyhow::bail!(
                    "CAIRN_REPLICATOR=mirror but CAIRN_PG_URL is set — the mirror \
                     keeps its own in-memory read model and must not point at \
                     Postgres. Unset CAIRN_PG_URL."
                );
            }
            let (handle, mut repl) = cairn_infra::MirrorHandle::open();
        ...
        other => {
            anyhow::bail!(
                "unknown CAIRN_REPLICATOR value: {other} (expected 'fake', 'pg', or 'mirror')"
            );
        }
    }
    ...
    if cfg.protect_metadata {
        if cfg.sync_auth == "none" {
            ...
            warn!(
                "CAIRN_PROTECT_METADATA is set but CAIRN_SYNC_AUTH=none — \
                 GET /schema stays effectively open (anonymous auth accepts any \
                 caller); only GET /rules is actually protected"
            );
        } else {
            info!("metadata protection: GET /schema requires sync auth, GET /rules requires CAIRN_ADMIN_TOKEN");
        }
    } else {
        info!("metadata protection: off (CAIRN_PROTECT_METADATA unset) — GET /schema and GET /rules are unauthenticated");
    }
    let ws_origins = parse_origin_list(&cfg.ws_origins);
    if ws_origins.is_empty() {
        info!("ws origin check: CAIRN_WS_ORIGINS unset — /sync accepts any origin");
    } else {
    ...
    if cfg.replicator == "pg" {
        ...
        let snapshotter: Arc<dyn cairn_application::ports::SnapshotSource> = Arc::new(
            cairn_infra::PgSnapshotter::new(&cfg.pg_url).with_max_rows(cfg.snapshot_max_rows),
        );
    ...
    if let Some(col) = tenant_col {
        if cfg.replicator == "pg" && !rules_tables.is_empty() {
            match cairn_infra::snapshot_source::audit_tenant_column(&cfg.pg_url, col, &rules_tables)
                .await
            {
                Ok(audit) => {
                    for table in &audit.columnless {
                        warn!(
                            ...
                            "synced table has NO tenant column: snapshots skip the \
                             ...
                             not, add the column or fix CAIRN_TENANT_COLUMN"
                        );
                    }
                    for table in &audit.missing {
                        warn!(
                            ...
                            "ruleset syncs a table that does not exist in PG \
                             (public schema) — usually a typo in cairn_rules.toml"
                        );
                    }
    ...
    // CAIRN_REPLICATOR=mirror — no handle, no route (the mount-time shape of
    ...
    // "ws" (default): sync sessions ride HTTP/WS on CAIRN_BIND. "iroh": an
    ...
    // direct-dialing ws client) binds CAIRN_BIND in BOTH modes.
    #[cfg(feature = "iroh")]
    if cfg.transport == "iroh" {
        // ADR-0041 D8: env-only operator knob — read only in iroh builds
        // under CAIRN_TRANSPORT=iroh; kept out of clap so non-iroh binaries
        ...
        let relay_url = cairn_infra::iroh_sync::parse_relay_url(
            cairn_infra::env::var("CAIRN_IROH_RELAY_URL")
                .ok()
                .as_deref(),
        )
        .map_err(anyhow::Error::msg)?;
        if let Some(url) = &relay_url {
            info!(relay = %url, "iroh sync: CAIRN_IROH_RELAY_URL set — self-hosted relay replaces the n0 default fleet");
        }
        let endpoint = cairn_infra::iroh_sync::bind_sync_endpoint(relay_url)
            .await
        ...
        info!(
            ...
            http_bind = %cfg.bind,
            dial_url = %url,
            "Cairn sync server listening — clients dial the iroh:// URL"
        );
    ...
    if cfg.transport == "iroh" {
        anyhow::bail!(
            "CAIRN_TRANSPORT=iroh but this server was built without the iroh feature \
             (cargo build -p cairn-server --features iroh)"
        );
    }
    ...
    info!(%addr, ws_path = %cfg.ws_path, transport = %cfg.transport, "Cairn HTTP surface listening");
...
/// `CAIRN_INSECURE_ANONYMOUS=1` — the universal shell/compose convention, and
...
/// `CAIRN_INSECURE_ANONYMOUS`). Only a **loopback** address is safe; note that
...
/// cairn-pushd's `/v1/send` (whose `data` is an arbitrary map).
///
/// ponytail: one key, not a map, because `CAIRN_PUSH_TABLES` is a
...
fn push_route_data(
    ...
    if let Some(route) = route {
        if !route.starts_with('/') {
            anyhow::bail!(
                "CAIRN_PUSH_TABLES: route {route:?} must start with '/' \
                 (table:visible@/orders/{{id}}:<title>:<body>) in {entry:?}"
            );
        }
        if route.chars().any(char::is_whitespace) {
            anyhow::bail!("CAIRN_PUSH_TABLES: route {route:?} must not contain whitespace");
        }
        data.insert(PUSH_ROUTE_KEY.to_string(), route.to_string());
    }
    cairn_infra::push::validate_data(&data)
        .map_err(|e| anyhow::anyhow!("CAIRN_PUSH_TABLES: {e} in {entry:?}"))?;
    Ok(data)
}
        ...
        let template = match parts.next() {
            ...
            Some(rest) => {
                ...
                if route.is_some() && !matches!(mode, "visible" | "action") {
                    anyhow::bail!(
                        "CAIRN_PUSH_TABLES: only visible/action entries take an @route \
...
async fn schema(
    ...
    // operator does not necessarily. Off unless CAIRN_PROTECT_METADATA is set,
    // because gating it breaks `cairn pull` and any Flutter app on an older SDK.
...
/// Boot-time resolution of `--protect-metadata` / `CAIRN_PROTECT_METADATA`,
...
mod rules_handler_tests {
    ...
    async fn rules_handler_reports_mode_and_tables() {
        ...
        let Json(body) = rules_handler(State(state), axum::http::HeaderMap::new())
            .await
            .expect("rules read is open while CAIRN_PROTECT_METADATA is unset");
...
/// Map the `--slot-max-lag` knob onto an [`cairn_application::EvictionPolicy`].
...
fn eviction_policy(slot_max_lag: u64) -> cairn_application::EvictionPolicy {
    if slot_max_lag > 0 {
        cairn_application::EvictionPolicy::new(slot_max_lag)
    } else {
        cairn_application::EvictionPolicy::disabled()
    }
}

/// ADR-0043: the `CAIRN_SLOT_MAX_LAG` default is a security decision (v0.2.0
...
mod slot_max_lag_tests {
    ...
    fn default_is_one_gib() {
        let cfg = Config::parse_from(["cairn-server"]);
    ...
    fn zero_means_unbounded() {
        let cfg = Config::parse_from(["cairn-server", "--slot-max-lag", "0"]);
    ...
    fn explicit_threshold_is_honoured() {
        let cfg = Config::parse_from(["cairn-server", "--slot-max-lag", "4096"]);
...
mod anonymous_bind_guard_tests {
    ...
    /// `=1` is what the refusal message, the compose file, and `cairn dev`
