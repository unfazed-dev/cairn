...
//! [`doorbell`](crate::doorbell) the wake-up; `cairn_core::PullCursor` owns the
...
/// A device syncing straight from Postgres — no Cairn server anywhere.
///
...
pub struct DirectClient<S> {
    ...
    /// of the database — it begins where `cairn link` installed the trigger —
