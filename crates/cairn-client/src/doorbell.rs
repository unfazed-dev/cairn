...
//! `cairn doctor` checks the setting — a policy alone is not a boundary.
...
impl DoorbellConfig {
    ...
    /// `cairn:` prefix keeps Cairn's topics from colliding with the app's own.
    pub fn new(
        base_url: &str,
        apikey: &str,
        scope: &str,
    ...
    /// The topic being listened on (for logs and `cairn doctor`).
    #[must_use]
...
/// split `cairn-ffi-wasm`'s `transport.rs` uses (frame logic pure, socket owned
/// by the platform).
#[must_use]
...
mod tests {
    ...
    fn any_broadcast_is_a_ring_whatever_it_carries() {
        ...
        for body in [
            r#"{"topic":"realtime:cairn:t","event":"broadcast","payload":{"event":"cairn","type":"broadcast","payload":{}}}"#,
