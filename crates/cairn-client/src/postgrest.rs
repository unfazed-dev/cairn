...
//!
//! The I/O half of [`cairn_core::pull`]. Core decodes, groups by `xid` and
//! advances the horizon; this makes the POST and drives the paging loop. The
//! browser's half of the same seam is `fetch` inside `cairn-ffi-wasm`'s Worker
...
use cairn_core::{ApplyEngine, Horizon, PendingWrite, PullCursor, PullError, Storage, WriteOp};
...
impl PostgrestSource {
    ...
    /// The `rpc/cairn_pull` endpoint (for logs and `cairn doctor`).
    ///
    /// The function lives in `public` under a `cairn_` prefix, not in the
    ...
    /// keeps the log table off the REST API entirely — `cairn link --mode
    ...
    /// own apply loop — the conformance suite, `cairn doctor` — can exercise
    ...
    /// and [`cairn_core::PullCursor::apply_snapshot`] is what turns the body
    /// into storage writes.
    ///
    /// # Errors
...
/// (`cairn_core::PendingWrite::pk` documents it). A per-table pk column is a
/// `cairn link` concern — it already has to read the schema to generate
...
impl PostgrestSource {
    ...
    ///
    /// The row never travels through a Cairn server, so the *only* thing
    ...
    /// one `cairn link --mode direct` must emit after `pull`.
