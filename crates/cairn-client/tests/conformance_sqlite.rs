...
//!
//! `cairn-core`'s own test runs the same cases against `InMemoryStorage`. Both
...
//! or a real `DELETE` can. The browser Worker leg (`cairn-ffi-wasm` over OPFS)
...
fn sqlite_storage_conforms() {
    let covered = cairn_core::conformance::run_all(|| {
