...
//!
//! Direct mode has two client implementations (`cairn-client` on rusqlite,
//! `cairn-ffi-wasm` in the browser Worker) and server mode shares the apply
...
///
/// ```no_run
/// # use cairn_core::{conformance, InMemoryStorage};
...
fn a_snapshot_reaps_rows_deleted_while_away<S: Storage>(storage: S) {
    ...
    assert_eq!(
        out.horizon.as_ref().map(cairn_horizon_str),
        Some("400".to_string()),
...
fn cairn_horizon_str(h: &crate::Horizon) -> String {
    h.as_str().to_string()
}
