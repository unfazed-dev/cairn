...
Cairn's four wedges (audited July 2026):

1. **A Rust server.** The server — the replicator, the sync router — is pure Rust (tokio + axum). Cairn's proven number: **2,618,601 ops/sec aggregate fan-out @ 1k clients, 0.00% drops** (eval-only — `FakeReplicator` on loopback, median of 3, 2026-09-02; the full real-PG → client-apply path is a separate figure, ~34.8k–36.2k rows/sec sustained — see [`benches/results/RESULTS.md`](../benches/results/RESULTS.md)). The Week-1 baseline of 142k is preserved as historical.
2. **Cairn is Apache-2.0 today** — OSI-open server, core, and SDKs; no change date, no no-compete clause.
3. **Write-back without endpoints.** ElectricSQL is read-path only. **Cairn's direct write-back** (ADR-0013) writes to your Postgres for you — no customer-built endpoints. **Honest trade-off:** because cairn writes directly as a least-privilege `BYPASSRLS` role, it **bypasses Supabase RLS** and substitutes its own authz (JWT ADR-0010 + table allowlist ADR-0013 + tenant-scope ADR-0018 — one column, coarser than arbitrary RLS). cairn wins for single-tenant / simple-tenant-scoped apps; for apps whose security model **is** complex per-user RLS, a split-write path through Supabase's Data API is the better fit. See [`docs/SECURITY.md`](SECURITY.md).
4. **Cairn self-host is free, full-featured, and unlimited.**
...
**Cairn offers direct write-back:** you give Cairn a Postgres connection + declarative **write rules** (which columns, which auth scope, upsert vs. insert), and Cairn applies queued client mutations to Postgres with **transactional conflict detection** (version/column-etag checks) and applies your chosen merge strategy. A `function` mode remains for anyone who wants full control. Most teams never write a backend mutation endpoint again.

### Front 3 — **Rust Server Throughput** 🏆 *performance moat*
> **Claim: *"Faster server, lower tail latency, a fraction of the memory — proven on Cairn's own aggregate fan-out benchmark."***

**Cairn's server is pure Rust (tokio + axum)**, parsing `pgoutput` via `pgwire-replication`, fanning out to thousands of concurrent WebSocket clients with per-connection backpressure, measured at 2,618,601 ops/sec aggregate fan-out @ 1k clients, 0.00% drops (median of 3 passes, 2026-09-02, eval-only; the pre-fix figure was 833,307). We publish continuous benchmarks and **make the benchmark repo public** so the claim is auditable — see [`benches/results/RESULTS.md`](../benches/results/RESULTS.md).

### Front 4 — **Truly Open (Apache-2.0)**
> **Claim: *"Apache-2.0, end to end. Server included. No FSL trap, no 2-year wait, no no-compete clause."***

Enterprise legal treats source-available-with-restrictions as procurement friction. **Cairn is Apache-2.0 across server, core, and SDKs.** This is the single biggest non-technical wedge: it makes us the morally-and-legally clean default.
...
**Cairn ships Flutter, React Native, Web (WASM/OPFS), Node/Electron, and native iOS/Android from one Rust core**, all first-class from day one, with CI on every platform. We meet developers where they are.
...
*Dynamic sync itself* is table-stakes now, not a Cairn-only feature. What still differentiates Cairn: **cursor-based, incremental** resume with no full-reprocessing cliff, and a predicate-evaluation engine purpose-built for it (ADR-0012).
...
**Why it's a moat:** it removes the need to build and host an upload endpoint. Combined with Front 6 (tiered conflict resolution), Cairn can honestly say: *"point us at your Postgres; we'll handle offline reads AND writes."* That's the magic that wins demos.
...
**The pitch:** Cairn's Free allows **10,000 peak devices**, and Pro is **base + flat data + $0.50/million ops** — know your bill before you ship. For a write-heavy B2B SaaS doing 100M sync ops/mo, Cairn Pro ≈ $49 + $50 = **~$99/mo**. That's a land-grab price; we win on volume, not margin-per-op.
...
### 8.2 The narrative
> *"ElectricSQL gave up on offline writes. Zero is web-only and disabled offline writes. Supabase can't do offline (yet — they bought Triplit). **Cairn is the open, Rust-fast one that does it all — no write-back endpoints, no lock-in, free self-host.**"*
...
1. **Week 1 — prove Front 3 (Rust throughput).** Stand up `cairn-server` reading PG logical replication via `pgwire-replication`; benchmark a pure fan-out to N WebSocket clients. **Goal: a public, auditable chart.** This chart funds everything.
