# Web SDK best practices Cairn can apply — 2026-09-21

Scope: the two web paths ratified in the 2026-09-21 grill (see
`docs/plans/cairn-integration-tauri-flutter-push.md` §Grilled decisions):
(A) Flutter-web via the ADR-0036 engine, (B) `@cairn/web` for JS/TS clients.
Both share one Worker design (`cairn.worker.js` + `sqlite_wasm_glue.js`, duplicated
in `sdk/cairn_web/worker/` and `sdk/cairn_flutter/web/cairn/`), so every item below
...
## What Cairn already gets right (keep)
...
1. **Now (≈30 lines):** Web Locks leader election in the Worker —
   `navigator.locks.request("cairn:" + dbName, { ifAvailable: true }, …)`. Loser
...
2. **Next:** `SharedWorker` variant of `cairn.worker.js` (same file, `onconnect`
   fans ports out). Chrome/Firefox/Safari ≥ 16 support SharedWorker; Android Chrome
   does not, so keep the dedicated-Worker path as fallback. Cairn's Worker already
...
when local mutations are outstanding. Cairn already exposes `pending` via
...
### 4. `@cairn/web` is not consumable from npm as packaged (P1 for path B)
...
- Construct the Worker the way Vite (and webpack 5) statically detect:
  `new Worker(new URL("./worker/cairn.worker.js", import.meta.url), { type: "module" })`.
...
- Flutter `--wasm` builds need `Cross-Origin-Opener-Policy: same-origin` and
  ...
  the JS build does not. sahpool needs neither, so Cairn imposes no header
...
- Flutter Wasm does not run on iOS browsers at all (WebKit WasmGC bug), and
  ...
  kit apps as the JS build until that clears; the Cairn Worker is plain JS and
  unaffected either way.
- `sdk/cairn_flutter/README.md` platform table still says "Web: punted" while
  ADR-0036 says shipped and Playwright-green. Fix the row.
- `kit/cairn` has no web-engine wiring; that is the actual A deliverable.

### 6. Dedupe the Worker (P2, hygiene)

`cairn.worker.js` and `sqlite_wasm_glue.js` exist twice with near-identical
comments. One source under `sdk/cairn_web/worker/`, copied into
`sdk/cairn_flutter/web/cairn/` by a script or symlink, before items 1–3 land twice.

## Non-issues confirmed

- **First-table-only checkpoint** (`crates/cairn-ffi-wasm/src/lib.rs:1320`): LSN is
...
- **WebSocket vs HTTP streaming:** PowerSync defaults to HTTP streaming on web and
  says there is "no compelling reason" for WebSockets there. Cairn's JSON-over-WS is
  fine; revisit only if a consumer sits behind a WS-hostile proxy.
- **COOP/COEP:** not required by sahpool. Do not add them for Cairn's sake.

## Version drift noticed on the way

| pin | current | where |
|---|---|---|
| `flutter_rust_bridge =2.13.0-beta.5` | `2.13.0` stable | `sdk/cairn_flutter/pubspec.yaml`, `rust/Cargo.toml` |
| `@sqlite.org/sqlite-wasm 3.53.0-build1` | `3.53.4-build1` | `sdk/cairn_web/package.json` |
| "native assets are an opt-in flag" | build hooks are first-class in Dart 3.13 docs | `sdk/cairn_flutter/README.md:283` |
