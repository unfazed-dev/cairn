...
query**, justified by "a cold-fresh-stream-per-`watch()` storm risk at cairn's
...
The 2026-07-13 reactive-query redesign locked `CairnDatabase` as the SQL-core sync
...
A cold-fresh-`Stream`-per-call `watch()` is common elsewhere; cairn's is already hotter
