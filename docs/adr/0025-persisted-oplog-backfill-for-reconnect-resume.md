...
Five web-research agents (2026-07-19) converge on the industry convention for Postgres-logical-replication engines: **backfill the op-stream** is the correct mechanism (ElectricSQL ships this way); **filtered-snapshot `WHERE updated_at > X`** is the textbook anti-pattern (drops deletes); **hard-delete is correct** (pgoutput already carries DELETEs via replica identity — tombstones are redundant server-authoritative); and **every production engine persists its op-log to disk/DB** (CouchDB by-seq, ElectricSQL shape logs). Bounded in-memory retention is correct but restart-fragile (deploy → thundering herd of re-snapshots).

## Decision

Build the **persisted operation-log backfill** — a single-stream subset of a bucket-partitioned op-log model. Four mechanisms:
...
- **Negative — write-amplification:** every WAL event now also writes a `cairn_oplog` row. `make bench` must re-measure the 142k ops/sec moat claim; ship before/after or revert (Tier-5 precedent, `docs/ROADMAP.md`). Batched per-commit writes mitigate. **Measured 2026-08-05 (slice 6 closed):** real-PG harness `crates/cairn-infra/tests/e2e_pg_write_amp.rs` proves amplification is **exactly 1:1** (`amp=1.000`, 200 source WAL events → 200 `cairn_oplog` rows) with **`oplog_dropped=0`**. The fan-out-side op-log cost was already shown invisible on the eval bench (833,305→833,307, `RESULTS.md`); the real-PG INSERT is off-loop and does not regress the fan-out ceiling. See `RESULTS.md` for the honesty-contract framing (fan-out ceiling ≠ real-PG ingest; never compared).
...
- **Divergence from bucket-partitioned designs (deferred, cited):** no per-bucket partitioning + no per-bucket checksums. Those are the multi-tenant fan-out efficiency that powers the 142k ops/sec @ 1k clients moat — the scalability/moat machinery, tracked separately, not conflated with the resume primitive ratified here.
