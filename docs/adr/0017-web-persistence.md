...
- **Option (2) wa-sqlite** — COOP/COEP tax with no compensating advantage over
  `opfs-sahpool` for cairn's single-writer model. It's the right choice for a
  custom-JS VFS layer built before sahpool matured; cairn has neither
  constraint.
...
> (`crates/cairn-ffi-wasm/src/lib.rs:496-566`: `enqueue` → `apply_local` →
...
OPEN its `Err` — `"cairn write: WebSocket send failed (socket not OPEN)"` — is
...
This is `CairnSocket.write` — the live browser transport — specifically. The
`CairnClient.write` on the `index.js` facade is a different surface: it feeds the
