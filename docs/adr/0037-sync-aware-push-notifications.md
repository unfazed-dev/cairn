...
requirement. Resolution: for tables listed in push config (`CAIRN_PUSH_TABLES`),
...
so any richer type has to be stringified anyway — cairn stringifies nowhere
...
`message_type`, `notification`, `google.*`, `gcm.*`; cairn's own `title`,
`body`, `category`, `table`, `lsn`) and caps the map at 1024 serialized bytes
against APNs/FCM's 4096-byte ceiling.

Two producers: `cairn-pushd`'s `POST /v1/send` takes the whole map, and
`CAIRN_PUSH_TABLES` grows one sugar — `orders:visible@/orders/{id}:…` sets
...
direct-mode `CAIRN_PUSH_TABLES`, one row per visible table. A change to such a
...
The rows come from `cairn link --visible <entry>`, where `<entry>` uses the
`CAIRN_PUSH_TABLES` visible/action grammar, so one line of config works in
