...
## Addendum: frb-generated `unsafe` in `sdk/cairn_flutter/rust` (2026-07-12)

The Flutter SDK's glue crate (`sdk/cairn_flutter/rust`) is generated by
flutter_rust_bridge, whose Dart-FFI codegen model emits `unsafe { }` blocks and
`#[unsafe(no_mangle)]` exports in `frb_generated.rs`. Unlike wasm-bindgen
(which lets `cairn-ffi-wasm` keep `#![forbid(unsafe_code)]`), this is inherent
to frb and cannot be hand-edited away.

Decision: the glue crate stays **outside the Cargo workspace** (its own empty
`[workspace]` table), so the workspace-wide `unsafe` ban continues to hold for
every workspace member; the generated-FFI exception is confined to this one
non-member crate and is machine-generated code only — hand-written `unsafe`
remains forbidden everywhere, including there. CI covers the crate's Rust via
the flutter job (clippy -D warnings + cargo test).
