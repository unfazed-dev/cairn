...
- **Status:** **Accepted (shipped)** — corrected 2026-07-30. This read "Deferred (Phase 4 —
  ...
  reader who checked here was told the headline feature did not exist. As built: `PgWriteBack` in `cairn-infra`, gated by `CAIRN_WRITE_TABLES`
  (`cairn-server/src/main.rs:112`, empty by default), covered by `e2e_pg_writeback.rs` +
...
+ host your own upload endpoint. Cairn
