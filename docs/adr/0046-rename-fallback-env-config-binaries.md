...
`cairn_infra::env` is the only sanctioned process-env reader:
...
the prefix scan in `cairn push`.

The warning is `eprintln!`, once per legacy name, not `tracing`: clap
resolves env before the subscriber exists, and `CAIRN_LOG` is itself one of
those args.

### 2. Config files and dirs: resolve once, edit in place

`cairn_infra::config_path::resolve(dir, name, legacy)` returns the primary
...
Explicit paths (`--rules-file`, `CAIRN_PUSHD_DB`, …) are taken as given, with
no fallback.
...
- **Fallback added.** These are operator- or user-facing runtime reads:
  - the Supabase push function's `CAIRN_PUSH_SECRET` (silent: it runs once
    per request);
  - the Flutter build hook's `CAIRN_FLUTTER_CARGO_FEATURES` (silent);
  - `sdk/cairn_react_native/scripts/build-ios-staticlib.sh`'s `CAIRN_PROFILE`
    (warns);
