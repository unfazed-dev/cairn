...
`try_send` and drop-on-full (`crates/cairn-infra/src/router.rs`). A session that
...
Cairn's server→client plane carries **state, not operations**. Three facts, verified
...
2. The client's apply is already last-writer-wins gated on LSN
   (`cairn-client/src/sqlite.rs:649`): `DO UPDATE SET payload = excluded.payload
...
(`CAIRN_RESYNC_SIGNAL`, currently default off) to on by default. Shipping conflation
...
- **The durable plane is untouched.** The op-log is written at the `FanOutService::run`
  ...
  independent of what the live plane conflates. This is what lets Cairn hold both
...
be cited as Cairn server limits either before or after this change. A two-host harness is
...
`CAIRN_RESYNC_SIGNAL` stays default-off. The decision above calls for flipping it, but
...
`cairn-bench` computes `drop_rate = 1 - delivered / (events × clients)` where `delivered` is
the **client-side frame count** (`crates/cairn-bench/src/main.rs`). Conflation's entire
...
1. **`cairn-bench` hardcoded `distinct_keys: 0`** — every event gets a brand-new row, so
   ...
   had `CAIRN_FAKE_KEYS`, defaulting to 50; the bench never did.
...
TokioEventSink::deliver  →  DeliveryDecision::Superseded
  ...
  → cairn-bench: drop = attempted − delivered − superseded
```
...
**penalty function matters more than raw age**; Cairn wants the threshold form `1(Δ > d)`, not
the average.
...
swung 27.11% → 1.19% across two runs. **Do not cite the 100k tier as a Cairn server limit.**
