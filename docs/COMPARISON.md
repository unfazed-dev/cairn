# Cairn — How We Compare (updated July 2026)

> *What Cairn's benchmark numbers actually measure, why each number is labeled by denominator, and the positioning wedges that hold.*
...
3. **Write-back without customer-built endpoints** (Cairn's direct write-back, ADR-0013) vs ElectricSQL's read-only path.
...
## 1. Cairn's numbers — every one labeled

Cairn's benchmark (`cairn-bench`) measures its Rust server's fan-out throughput.

**Every Cairn number is labeled by what it measures, and only same-denominator pairs are compared** — against each other or against any other engine's figure:

| Cairn number | Denominator |
|---|---|
