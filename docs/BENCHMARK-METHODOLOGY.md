...
3. Obtains an in-process handle to the server's `FanOutService` and constructs a `FakeReplicator` that emits `M` synthetic events as fast as the router will accept them (the router's backpressure is the rate limiter).
   - **`CAIRN_FAKE_EPS` / `CAIRN_FAKE_KEYS` do not apply here.** Those bound the `cairn-server` *binary's* dev default (A10, ADR-0027); `cairn-bench` builds its own `FakeReplicatorConfig` (`crates/cairn-bench/src/main.rs:415`), leaving both knobs at `0` = unpaced, monotonic keys unless `--rate` / `--distinct-keys` say otherwise. The measured ceiling is unaffected by them — and must stay that way, since pacing would cap the very number this document defines.
...
One run is not a result. `cairn-bench` runs **N = 5 measured repetitions per tier** (`--reps`), preceded by **one discarded warm-up per tier** (`--warmup-reps`), and reports the MLPerf trimmed mean: fastest and slowest dropped, mean of the rest.
...
- **START gate.** A *build* may run under any host load. A *measurement* only
  ...
  and `/tmp/cairn-bench.lock` is free.
...
Compare only same-stage, same-units figures from any other engine (§1). An aggregate fan-out figure is never divided by another engine's replication-ingest rate, per-client sync rate, MB/sec or txn/sec. Every Cairn figure carries its scope — eval-only (synthetic replicator on loopback) or full path (real Postgres → client apply) — and the two are never compared against each other.
