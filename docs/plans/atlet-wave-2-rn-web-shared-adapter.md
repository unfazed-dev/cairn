...
- **In scope:** a shared TypeScript `SyncAdapter` port (mirroring
  ...
  app, each wired to a CairnAdapter for their respective SDKs, running the
...
2. **One shared TS adapter file consumed by two app targets, or two
   ...
   web for cairn; other sync SDKs split RN vs. IndexedDB backends similarly). "Shared TS
...
3. **Web durability:** per `[[cairn-adr-audit-2026-07-30]]` (project
...
3. RN CairnAdapter (own conformance-item assignment,
   explicit per the retro's rule above).
4. Web CairnAdapter (own conformance-item assignment;
