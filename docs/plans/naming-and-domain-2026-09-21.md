# Naming + domain: what actually needs a domain, and cairn vs qairn
...
| **Cairn Cloud** (hosted) | Yes, eventually | Not a launch blocker. Not built yet |
...
### Why a Cloudflare Worker cannot host Cairn Cloud either
Cairn Cloud is a long-lived stateful Rust process: it holds a Postgres
...
## 2. cairn vs qairn — verified registry state (checked 2026-09-21)

| Registry | `cairn` | `qairn` |
|---|---|---|
...
The blocker is `cairn-cli`. The product's primary UX is a CLI called `cairn`,
and `cargo install cairn-cli` already installs a stranger's crate at v0.1.6.
`cairn` itself is squatted at v0.0.0, so the flagship crate name is gone
permanently — no amount of waiting gets it back. Publishing as
`cairn-sync-cli` that produces a binary named `cairn` is a papercut shipped
...
it owns its search results outright; "cairn" competes with a Scottish terrier
breed, Cairn Energy, Cairn Capital and cairn.info.

Cost of the rename, honestly: it is mechanical but wide — crate names, the
`CAIRN_*` env vars, the Maven artifactIds, the Flutter/RN package names, the
...
### If you keep `cairn`
Workable but compromised: publish `cairn-server` (free), and find free names
for the core and CLI crates. The Maven groupId `io.github.<user>` and the
`@cairn` npm scope and all four pub.dev names are unaffected either way.
