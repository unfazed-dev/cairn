...
- React Native SDK verified: `tsc --noEmit` clean, jest 33/33 across
  `CairnClient` / `push` / `signout` / `watch`. It is healthy — only unpublished.
...
|---|---|
| `cairn` | **TAKEN** — 0.0.0, "Build-gated version control for Rust projects" |
| `cairn-core` | **TAKEN** — 0.1.0, "Core types, storage, and query engine for Cairn knowledge provenance" |
| `cairn-cli` | **TAKEN** — 0.1.6, "Backpac Agent-Native CLI for autonomous settlement workflows" |
| `cairn-domain`, `cairn-application`, `cairn-infra`, `cairn-server`, `cairn-client`, `cairn-ffi-wasm`, `cairn-license`, `cairn-push`, `cairn-bench` | free |

`cairn-core` and `cairn-cli` are both crates we ship. This needs a naming
...
1. **Prefix only the two colliding crates** — `cairn-sync-core`, `cairn-sync-cli`.
   Smallest diff; leaves the other seven names as-is. Downside: inconsistent
   prefix across the published set.
2. **Prefix the whole set** — `cairn-sync-*`. Consistent, larger diff, and the
   binary name in `cairn-cli` (`cairn`) still collides with the `cairn` crate if
...
Not free on any registry, unrelated to the above: `cairn` on pub.dev,
`cairn_flutter` on pub.dev, and the `@cairn` npm scope are all **free/empty**
...
**`@cairn/web` was publishing without its wasm core.** `wasm-pack` writes
...
out as 11 files / 108 kB with the 269 kB `cairn_ffi_wasm_bg.wasm` silently
...
`sdk/cairn_web/check-pack.cjs` (`npm run check:pack`) — verified to exit 1 and
name the missing files when the fix is removed.

**`@cairn/node` had no `files` allowlist.** Its tarball carried `Cargo.toml`,
`Cargo.lock`, `build.rs`, 57 kB of `src/lib.rs`, and
`.claude-flow/data/pending-insights.jsonl` — a local agent-tooling artifact
leaking into a public package. It also ships one 8.5 MB `cairn_node.node` built
...
one-line revert, and it is the only thing now preventing `@cairn/node` from
being published.

`@cairn/react-native` (20 files / 128.8 kB) and `@cairn/capacitor`
...
`sdk/cairn_kotlin/android/build.gradle.kts` had **no `maven-publish` plugin, no
...
toolchain drift is closed. `sdk/cairn_kotlin/android` and
`sdk/cairn_react_native/android` now run **Gradle 9.7.1 + AGP 9.4.1** with
...
- **Source sets.** Built-in Kotlin reads the `kotlin` source set only. The
  ...
  cairn-kotlin, 106 for cairn-react-native.)

**Not bumped, deliberately.** The Flutter Android projects
(`apps/atlet/flutter/android`, `sdk/cairn_flutter/`) pin AGP 9.0.1 /
...
Flutter moves. `sdk/cairn_capacitor` pins AGP 8.13.0 to match the Capacitor 8.5
...
`docs/IDENTITY.md` and the `CAIRN-IDENTITY-PENDING` grep token record this.
...
4. Create the `@cairn` npm org (the scope is empty, not necessarily claimable —
...
7. `git pull`, then `cd sdk/cairn_flutter && flutter pub publish --dry-run &&
...
Show HN timing and Cairn Cloud alpha stay where the roadmap put them: founder
...
- **The 100k cliff attribution was retracted.** `cairn-fanout-walk` measures the
...
(BLOCKER 2), the `@cairn` npm org, identity (BLOCKER 4 proper), the tag push,
Show HN timing, Cairn Cloud alpha.
...
`implementation("io.github.unfazed-dev:cairn-kotlin:0.2.0")` until that
coordinate exists there. It is the Kotlin/Android equivalent of npm for
`@cairn/web` and crates.io for the Rust crates; Cairn needs all three because
it ships to all three ecosystems.

A coordinate is `groupId:artifactId:version` — here
`io.github.unfazed-dev` : `cairn-kotlin` : `0.2.0`. The group id must be a
...
|---|---|
| `cairn-kotlin-0.2.0.aar` | the library itself (classes + the arm64 `.so`) |
...
- **No domain is needed to launch.** The Maven groupId gate is closed by
  ...
  `*.workers.dev` is Cloudflare's, not yours. A domain is a Cairn Cloud +
  marketing concern only, and Cloud is not a launch deliverable.
- **Registry state verified 2026-09-21, not remembered:** `cairn`,
  `cairn-core` and `cairn-cli` are all TAKEN on crates.io (`cairn` squatted at
  v0.0.0, so it is gone permanently); `cairn-server` is free; npm `cairn` is
  taken. Every `qairn*` name is free on crates.io, npm (bare and scope),
  pub.dev and as a Maven artifactId.
- **Recommendation: rename to `qairn`,** because `cargo install cairn-cli`
  ...
  the counter-argument (the cairn/trail-marker metaphor carries the launch
