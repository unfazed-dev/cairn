...
Which is exactly what ADR-0045 asserts about Cairn's row plane — every frame is a complete row
...
- **The penalty function matters more than raw age.** `g(Δ)` encodes the use case: a threshold
  ...
  function for periodically-checked data (a UI that repaints on a frame boundary — i.e. Cairn's
  actual consumer), exponential for control loops. Cairn wants the threshold form.

## The metric to build, and why it is cheap

`ReplicationEvent` carries no timestamp (`crates/cairn-domain/src/events.rs:128` — `lsn`, `op`,
...
2. **A run that hit its timeout must not report a number.** The 10k rung's `elapsed_secs: 120.00`
   is the `--timeout-secs` default, i.e. window expiry. `cairn-bench` should stamp
...
- **Absolute throughput at 50k–100k** → still needs the generator off the server's host.
  Unchanged: *do not cite the 100k tier as a Cairn server limit.*
