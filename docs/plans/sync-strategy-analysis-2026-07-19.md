...
| ↳ *optimistic writes + store-and-forward* (sub-pattern) | Offline *writes* | **cairn**, Replicache (mutator+rebase) |
...
| **Log-based / logical replication** (WAL stream) | Server-authoritative + offline, DB already exists | **cairn**, ElectricSQL, Datomic |
...
- cairn's 142k ops/sec @ 1k clients — from project memory / `benches/results/RESULTS.md`, **not re-verified this session**. The "35×" comparison is vs a competitor's published *service* ceiling, not a matched-load bench.
