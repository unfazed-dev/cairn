...
Systematic-debugging record for the 10k-client soak (`cairn-bench-10k 10000 5000 60`)
...
Instrumented the probe (`crates/cairn-bench/src/bin/probe_10k.rs`) with
...
2. **Host limit at ~9.2k loopback sockets on this Mac:** the fixed build
   ...
   4.17M attempted deliveries are counted against the probe. Not cairn code;
...
3. Official numbers: the 3-pass `cairn-bench` re-measure + 10k soak
   (`/tmp/cairn-remeasure3.sh` shape) must be re-run on the fixed build before
