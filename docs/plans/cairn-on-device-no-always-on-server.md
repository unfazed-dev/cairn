...
**`cairn-server` is therefore not optional for this product, it is the
...
### Shape B — `cairn lite`, direct to the backend's own API

No replication at all: the client speaks the backend's REST/realtime surface
(PostgREST + Realtime on Supabase) and keeps Cairn's local SQLite, outbox and
...
The topology is A1, so **`cairn-server` stays, and stops being apologised for.**
The README line:
...
> the only holder of a privileged credential. That is `cairn-server`.
...
3. **Shape B (`cairn lite`) as the zero-infrastructure trial**, with
   "~3,000 subscribers, or the moment you need server-authoritative scoping" as
   the documented trigger to move to a real `cairn-server`. *The upgrade path is
...
1. `cairn-infra` behind an `embedded` feature in `sdk/cairn_flutter/rust`
   (today it depends only on `cairn-client`, `cairn-core`, `cairn-domain`).
...
4. Credential story: a user-supplied connection string, entered or scanned at
   ...
   into the bundle — `cairn doctor` should fail loudly if it finds one.
