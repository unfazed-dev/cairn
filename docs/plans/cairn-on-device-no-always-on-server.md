# Cairn with no always-on server — the on-device shapes and what each costs

**Date:** 2026-09-22. **Status:** topology **decided** — one shared database per
app, shipped to an agency's clients, and **no Cairn server anywhere**. That
...
to involve another device that stays on all the time for Cairn to work."*

The constraint is legitimate and the current answer ("run cairn-server on a VM
...
**The correction.** Drafts 1–3 of this document treated "no Cairn server" as
...
replication is one implementation of Cairn's change-source port, not the
definition of Cairn.** ADR-0023 D4 already says so in as many words: the
...
**`cairn-server` is not a process you must keep alive. It is a library.**

`cairn-server` is a composition root over `cairn-infra`; the session logic lives
in `cairn-infra`'s transport module, not in the axum handler. `README.md`'s
...
The app's Rust lib links `cairn-infra` with the `pg` feature and holds the
...
**Works:** no second device, no hosting bill, full Cairn semantics (predicates,
...
**A1 is what needs `cairn-server`** — one process holding the single slot and
...
- **An abandoned slot is the owner's own.** No cross-user damage, and `cairn
...
server, no second device, per-install Cairn, full semantics, at any number of
...
need cairn-server; database-per-user → you don't.**
...
"Cairn on the device" — the *replicator*. A2 stays as a documented power-user
...
`cairn-server`, or yours, or a process you pay for. Supabase already runs one
...
cairn-server stops consuming its slot, `max_slot_wal_keep_size` is exceeded,
...
- **We have never measured cairn-server's memory footprint.** 256 MB is a
...
No replication anywhere on the client side. `cairn-client` keeps everything it
...
*source of frames* changes: instead of Cairn's `/sync` WebSocket, the device
...
is no Cairn process in the middle, for anyone, ever.
...
4. The realtime stream is a **doorbell**; every reconnect resyncs from the
   checkpoint before trusting a streamed frame (RxDB's `RESYNC`, and Cairn's own
   ADR-0037 doctrine).
5. The watermark is captured **before** the catch-up query, accepting duplicate
   delivery — free for Cairn, whose `(table_name, pk)` row images are idempotent.
...
Jepsen-verified. That is the strongest remaining argument for `cairn-server` —
...
and Appwrite do. A bare Postgres does not, and for that, `cairn-server` remains
the answer.
...
Cairn server for anybody, nothing centralised that the agency has to own.

It is also not a compromise version of Cairn. The device keeps the engine, the
...
**`cairn-server` stops being the entry point and becomes the upgrade path**, for
the cases that actually need it: a bare Postgres with no realtime service,
Cairn-evaluated predicates rather than RLS-plus-topics, LSN-exact replay,
conflation and backpressure under Cairn's control (ADR-0045), or a backend whose
realtime tier costs more than a $2 machine.

### The work

1. **A `ChangeSource` seam in the client.** `cairn-client` today speaks only the
...
4. **`cairn link --mode direct`** generates the SQL the client's DB needs: the
...
5. **`cairn doctor`** for this mode: trigger present, RLS policies present,
...
- Is the `updated_at` + soft-delete schema requirement acceptable to impose on
  ...
  a legacy schema.
- Does the agency's clients' data ever need Cairn-evaluated predicates, or is
