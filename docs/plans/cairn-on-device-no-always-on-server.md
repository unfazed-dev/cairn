...
- **A1 is the shape that needs `cairn-server`**, and it is also the shape most
...
> sessions — that is `cairn-server`. If each user owns their own database, you
> don't need it.
...
- If A2: who provisions the per-user database — the user by hand, or Cairn via
