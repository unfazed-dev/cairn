...
- **Correctness never waits on a push.** The snapshot horizon already makes
  ...
  *staleness until next open*, not a missed row. This is already true of
  cairn-server's doorbell (ADR-0037) — direct mode inherits the property, it
...
Edge Function can also ring the socket doorbell, and how `cairn doctor` can
...
and ≤4 days. Cairn's horizon already makes replay unnecessary — noted so nobody
builds it twice.
...
| observability | responses kept 6h in `net._http_response` | `select * from net._http_response where status_code >= 400 or error_msg is not null` is the `cairn doctor` query |
| reconfigurable | only on pg_net **v0.12.0+** | `cairn doctor` should report the version |
...
`cairn link` must refuse to install change-log triggers on the `net` schema.
...
```json
{
  "message": {
    ...
    "android": { "priority": "normal", "collapse_key": "<cairn key>", "ttl": "3600s" },
    "apns": {
      "headers": {
        "apns-priority": "5",
        "apns-push-type": "background",
        "apns-collapse-id": "<cairn key>"
      },
...
3. **`collapse_key` is a data-message tool** — notification messages ignore it.
   Cairn's `default_collapse_key(tenant, token)` (`crates/cairn-push/src/rail.rs:182`)
...
**The Android overflow path is already Cairn's re-snapshot path.** Non-collapsible
...
`crates/cairn-push/src/rail.rs:56` already knows this. `cairn.push_tokens`
...
than cairn-server has it today** — cairn's `SessionStore` tracks sessions for
...
Supabase's limit, not Cairn's. Worth stating plainly in the README rather than
...
Idle-but-connected devices need one cheap `cairn.heartbeat()` on a 60s timer
...
loop that doorbells every device once a minute forever. `cairn link` should
refuse to instrument it, and `cairn doctor` should assert it is not instrumented.
...
## What this means for cairn-server mode
...
cairn-server" would not be true.
...
| `pg_net` failures visible | `select * from net._http_response where status_code >= 400` | wire into `cairn doctor --mode direct` |
