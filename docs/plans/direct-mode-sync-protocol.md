...
**Companion to:** `cairn-on-device-no-always-on-server.md`, which argues *why*
...
writes through its own REST API. No Cairn server. Everything in `cairn-client`
...
**So:** `cairn link --mode direct` generates a `BEFORE UPDATE` trigger that
...
`cairn link` refuses a table without it.
...
Cairn already holds this position in its own words: push is a **"wake-up
...
free for Cairn specifically**: `cairn_data` is keyed `PRIMARY KEY (table_name,
pk)` and a frame is a complete row image, so re-applying a row is a no-op by
construction. Cairn's storage model already pays for this.
...
**So:** `cairn doctor` checks the setting, not just the policies. A project with
...
1. **`cairn-server`'s pitch should lead with consistency, not ops/sec.** It
   applies at commit boundaries with `txn_id` and `lsn` on every event
   (`crates/cairn-domain/src/events.rs`); direct mode has neither. The
...
keeping Cairn's durable outbox and writing through PostgREST is therefore a
...
1. `ChangeSource` seam in `cairn-client` (`client.rs` today speaks only `/sync`;
...
6. `cairn link --mode direct`: trigger + RLS policy + column generation, and a
   hard refusal for any table missing `updated_at` or `deleted_at`.
7. `cairn doctor --mode direct`: policies, the public-access setting, watermark
