...
`Storage`), `cairn_client::postgrest` (`rpc/pull` + the four write ops), and
`cairn_client::doorbell` (Realtime private channel, `vsn=1.0.0`). What is *not*
...
3. PostgREST exposes it at `/rest/v1/rpc/pull`. Supabase's custom-schema guide
   covers the setup: add `cairn` to "Exposed schemas" in API settings, then
...
5. Outbox drain → PostgREST write; the log trigger captures the echo, which the
   ...
   below.
6. `cairn link --mode direct` generates the schema, the per-table triggers, the
