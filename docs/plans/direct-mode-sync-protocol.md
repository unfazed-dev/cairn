...
Shipped: `cairn_core::pull` (`PullCursor`, `Horizon`, the xid8 checkpoint on
`Storage`), `cairn_client::postgrest` (`rpc/cairn_pull` + the four write ops),
`cairn_client::doorbell` (Realtime private channel, `vsn=1.0.0`), and
`cairn_cli::direct` — the generator behind `cairn link --mode direct` and the
verifier behind `cairn doctor --mode direct` — plus `cairn_core::conformance`
(one suite, run per platform) and the opt-in push path.

**The SQL below is no longer theory.** `crates/cairn-cli/tests/e2e_pg_direct_sql.rs`
...
**Companion to:** `cairn-on-device-no-always-on-server.md` (*why* direct mode).
...
Realtime. No Cairn server, no second box, every sync decision made on the
device.
...
Postgres itself rather than from a service. Cairn's own `ReplicationEvent`
already carries `txn_id` (`crates/cairn-domain/src/events.rs`), and
...
3. **It lives in `public` under a `cairn_` prefix, not in the `cairn` schema.**
...
also emit a RESYNC event each time the client reconnects." Cairn already holds
...
access' setting** in Realtime Settings". `cairn doctor` checks the setting.
...
2. **RLS has to be expressible on one table.** The log holds row images from
   ...
   work, and `cairn link` should refuse rather than guess.
...
5. ✅ Outbox drain → PostgREST write; the log trigger captures the echo, which the
   ...
   below.
6. ✅ `cairn link --mode direct` generates the schema, the per-table triggers,
   ...
   `crates/cairn-cli/src/direct.rs`; applied to real Postgres by
   `crates/cairn-cli/tests/e2e_pg_direct_sql.rs`. See "What the generator
   refuses" below.
7. ✅ `cairn doctor --mode direct`: the objects, the grants (including that
   `anon` may NOT execute `cairn_pull`), read-only policies, the per-table
   triggers, the Realtime policy, log growth and horizon lag —
   `cairn_cli::direct::inspect`, all `select`s, safe against production.
...
8. 🟡 One conformance suite both modes pass, **run per platform, not once** —
   `cairn_core::conformance` (feature-gated, four cases) is the suite;
   `cairn-core`'s own test runs it on `InMemoryStorage` and
   `crates/cairn-client/tests/conformance_sqlite.rs` runs it on rusqlite.
   **The browser-Worker leg is not wired up** — `cairn-ffi-wasm` has to export
...
9. ✅ Push, opt-in behind `cairn link --mode direct --push <url>`:
...
`cairn link --mode direct` reads `cairn_rules.toml` and emits one re-runnable
...
Atlet is a Supabase-backed **Cairn vs PowerSync** app, and its shape is the
...
- **`lib/engine_registry.dart`** — `enum Engine { cairn, powersync }` plus an
  `EngineRegistry` that hot-swaps them with a mutual-exclusion guard ("cairn and
...
`sdk/cairn_flutter/example` is the other Flutter app (a 6-table booking
dashboard on one `/sync` socket). It is the SDK-surface demo, not an engine
comparison: it dials `ws://127.0.0.1:8800` from `cairn dev` and has no adapter
...
`fixture-todo-smoke-live` and `fixture-todo-cairn-live-{up,down,proof}` all `cd`
...
`integration_test/cairn_live_test.dart` — so its wiring is recoverable from
...
`cairn link`'s default from server to direct is a second**, taken later, on the
...
1. **Join-based RLS** — `scope` covers tenant/owner; `cairn link` refuses the rest.
...
**Sequencing.** `cairn link --mode direct` ships behind the flag first — the
...
Three things cairn-server does for push today. Each needs a direct-mode answer:

| what the server does | where it lives now | direct-mode replacement |
|---|---|---|
| holds the APNs `.p8` / FCM service-account JSON | `Rails::from_env()`, `crates/cairn-push/src/rail.rs:122` | a Supabase Edge Function secret — **never the device** |
| decides *whom* to doorbell | `FanOutService::fan_out` enqueues one `PushHint` per matched **offline** account, `crates/cairn-application/src/fanout.rs:396` and `:450` | trigger on `cairn.changes` reads `scope`, selects the token rows for that scope |
...
`cairn-pushd` is unaffected and stays the answer for an operator who wants a Rust
...
`default_collapse_key(tenant, token)` (`crates/cairn-push/src/rail.rs:182`) is a
...
`public` scope. The revokes now name the roles. `cairn doctor --mode direct`
...
`createClient(..., { db: { schema: "cairn" } })`, and `cairn` is deliberately
...
contract the client has to honour: pull again. The harness polls; `cairn doctor`
