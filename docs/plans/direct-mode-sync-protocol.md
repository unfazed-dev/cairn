...
obtained here from Postgres itself rather than from a service. Cairn's own `ReplicationEvent`
...
- **`lib/engine_registry.dart`** — an `enum Engine` with a comparison-engine slot
  alongside `cairn`, plus an `EngineRegistry` that hot-swaps them with a
