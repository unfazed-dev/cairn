...
(the ratified client-SDK redesign plan, `cairn-cloud-trust-and-coverage.md`).
...
Closes the "competitors offer X cairn doesn't" gaps so the moat comparison is clean:
...
- **Dashboard** parity (the comparable-engine dashboard is cloud-only / paid-self-host;
  cairn ships one free for self-host — itself a wedge).
