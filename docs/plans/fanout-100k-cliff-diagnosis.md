...
Read: crates/cairn-application/src/fanout.rs, crates/cairn-infra/src/{router,
transport,store}.rs, crates/cairn-bench/src/bin/probe_10k.rs.
...
`benches/scripts/fanout-100k-diag.sh` (own container `cairn-linux-fanout-100k`,
own volumes `cairn-linux-target-fanout` / `cairn-linux-cargo-registry-fanout`,
honours `/tmp/cairn-bench.lock`). One container, two tiers, both with:
...
`cairn-fanout-walk` (`crates/cairn-bench/src/bin/fanout_walk.rs`) runs the real
...
- the 100k **in-process client tasks** — `cairn-bench-10k` runs the swarm in the
...
`Metrics` gained four diagnostic counters (`crates/cairn-application/src/ports.rs`),
written by `FanOutService` and read directly by `cairn-bench-10k`:
...
- **A comparison point for `cairn-fanout-walk`.** Same stage, same code, with
...
200 events (`cairn-bench-10k 2000 200 60 1 1`), host load1 ≈ 36 on 10 cores:
...
(`cairn-fanout-walk` says it does — 0.458 → 0.486 µs), starvation is proven and
...
is a measurement of the host, not of Cairn.
...
### The instrument
`cairn-fanout-walk` gained a `sink=wire` mode. The gap that mattered: the
...
**is** wired in `cairn-server` (`--ack-progress-interval` /
`CAIRN_ACK_PROGRESS_INTERVAL`, `main.rs:265`). What was wrong was its default:
...
`cairn-fanout-walk` calls `min_acked_lsn` **once, at the end, with the walk
...
### Shipped
`CAIRN_ACK_PROGRESS_INTERVAL` default **1 → 16** (`cairn-server/src/main.rs`),
...
was already wired into `cairn-server`; only its default was wrong.
...
falsification on that basis, and shipped `CAIRN_ACK_PROGRESS_INTERVAL = 16`. One
...
Shipped: `CAIRN_ACK_PROGRESS_INTERVAL` reverted to `1`. The safety test
