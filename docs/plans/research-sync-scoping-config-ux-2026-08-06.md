...
precedent for Cairn, since Cairn's predicates are also a live, per-session filter tree rather
...
server) — this mirrors the per-bucket invalidation pattern described above, adapted to Cairn's
per-session model.
...
- **Deploy semantics**: on `cairn rules apply` (SIGHUP), re-render every live session's compiled
  ...
  this is the Cairn-shaped version of the checksum-invalidation pattern described in §2.D, and
  it's free: Cairn has no bucket-log/compaction machinery to rebuild, so the "cost" of
