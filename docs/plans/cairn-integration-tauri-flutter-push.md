...
- **Track D — ADR-0041 ACCEPTED 2026-08-29** (merge `2e6cb9c`; decision memo `docs/plans/adr-0041-decision-memo.md`). Spike green: ws/iroh conformance parity re-run at `680852f`, iroh off-default everywhere, iOS/Android build viability verified. **Accept-gated items (now tracked work, conditions in ADR-0041 §Acceptance):**
  ...
  - D6. **Native `run_session` refactor** — kill the loopback-bridge ponytail (one loopback TCP hop per connection today, recorded in `cairn-server/src/main.rs`); iroh streams drive the session core directly via a small frame-io trait.
  - D7. **Flutter/tauri SDK wiring** — FRB bridge with the `iroh` feature enabled; iroh stays off-default in shipped artifacts until D5 + D8 land.
  - D8. **Self-hosted relay guidance + n0-fleet privacy note** in operator docs — before or with D7.
