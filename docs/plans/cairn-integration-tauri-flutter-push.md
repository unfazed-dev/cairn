...
- `sdk/cairn_tauri` — feasibility scaffold, NOT a polished SDK. One Rust file `sdk/cairn_tauri/src/lib.rs` (~1,156 lines), standard Tauri 2 plugin (`.plugin(tauri_plugin_cairn::init())`). Commands: connect, subscribe, write, query, checkpoint, watch, set_token, sign_out. No `.d.ts`, no JS/TS package, no docs beyond README invoke examples. **Zero push code** (no matches in `src/lib.rs`).
...
- C2. Reuse atlet as the reference: `lib/adapters/cairn_adapter.dart` (built alongside a comparison adapter during the pilot) and `lib/push/push_pilot.dart` (FCM mobile + VAPID web, `--dart-define=ATLET_PUSH_PILOT=true`, `cairnDoorbellBackgroundHandler` background isolate).
...
Session: `/grill-with-docs` on "is cairn usable in flutter/dart/tauri/web projects already".
...
`arxa/kit/cairn` pins `cairn_flutter` by git SHA `fabc1a1` (on origin); `arxa/desktop` is
the Tauri consumer; no JS/TS app in arxa imports `@cairn/web` yet.
...
3. **Tauri: fixture app first, multi-table lifted inside it.** `arxa/desktop` needs many
   tables, so the v1 one-table `CairnState` assert (`sdk/cairn_tauri/src/lib.rs:104`) is a
...
4. **Web = both, neither deferred.** (A) Flutter-web via the ADR-0036 engine — needs
   `kit/cairn` web wiring and a fix to the `cairn_flutter` README platform row that still
   says "Web: punted". (B) `@cairn/web` for standalone JS/TS clients. The additive wasm
   `subscribe()` already gives both paths multi-table; the first-table-only checkpoint
   (`crates/cairn-ffi-wasm/src/lib.rs:1320`) is correct because LSN is stream-global — leave,
