...
> |---|---|---|---|---|---|
> | `cairn_flutter` | **0.1.0** | ✅ | ✅ | ✅ | **publishable** |
> | `cairn_capacitor` | 0.0.0 | ❌ | ❌ | ✅ | blocked |
> | `cairn_dotnet` | 0.0.0 | ❌ | ❌ | ✅ | blocked |
> | `cairn_react_native` | 0.0.0 | ❌ | ❌ | ✅ | blocked |
> | `cairn_web` | 0.0.0 | ❌ | ❌ | ✅ | blocked |
> | `cairn_kotlin` | 0.0.0 | ❌ | ❌ | ❌ | blocked |
> | `cairn_swift` | 0.0.0 | ❌ | ❌ | ❌ | blocked |
> | `cairn_tauri` | 0.0.0 | ❌ | ❌ | ❌ | blocked |
> | `cairn_node` | 0.0.0 | ❌ (no `license` field either) | ❌ | ❌ | blocked, worst case |
>
...
> Maven. Four have no README at all (kotlin, node, swift, tauri). `cairn_tauri` has no
...
>
> **This does not block the Flutter+Supabase wedge** — `cairn_flutter` is the one that is
...
> any "install cairn for <platform>" claim.
>
...
| Concern | Reference SDK | Cairn | Gap |
|---|---|---|---|
...
Not everything should converge on that reference SDK. Two Cairn choices are real advantages and
must not be "fixed" into parity:

- **Zero-backend writes.** Comparable sync SDKs require the developer to implement a
  `BackendConnector`-style `uploadData()` and their own backend write endpoint + RLS. Cairn
...
| Error is a **first-class field** in the read/status surface | Instant (`{isLoading, error, data}`), a comparable sync SDK (`uploadError`/`downloadError`/`anyError`), TanStack | Cairn `SyncStatus` has no error field → A2 |
...
| SDK owns retry/backoff; app owns presentation | a comparable sync SDK (blocking FIFO upload queue, SDK handles retries) | Cairn already does this correctly — keep it |
...
3. **A comparable sync SDK requires a `BackendConnector`-style adapter** — the developer
   ...
   POSTs to their own backend API. Cairn requires **none** of this. Confirms the zero-backend
...
SDKs make you build the backend. Cairn's remaining gap is not capability, it is **telling
...
| what | before | after |
|---|---|---|
| versions | 8 packages at `0.0.0` | all `0.1.0` (matching `[workspace.package]` and `cairn_flutter`) |
...
1. **`sdk/cairn_tauri` never exposed `subscribe` to JS.** `generate_handler!` and `build.rs`
   ...
   *Why no test caught it:* the tauri slice is `cargo test`, which calls `CairnState::subscribe`
   directly and never crosses the command boundary. Recorded in the new README.
2. **`dotnet/Cairn.DotNet.csproj` was not well-formed XML** — two fatal errors: a mismatched
...
3. **`@cairn/react-native`'s own `npm run typecheck` failed** under its own strict tsconfig
...
4. **`sdk/cairn_web/README.md` understated a shipped capability** — it called the whole package
   a "reduced-scope feasibility proof" that "does NOT yet open a live WebSocket." The *browser*
   path (`pkg-web` → `CairnSocket`) opens a real WebSocket and is proven by
   `e2e/browser_live.spec.cjs`, which **is** the passing `web` slice. Only the Node facade is
   reduced-scope. Now documents both paths with a `CairnSocket` API table.
5. **`sdk/cairn_flutter/README.md` gave actively wrong advice** — "one active subscription per
   `Cairn` instance… use a second `Cairn.connect(...)`" for a second table. `subscribeTables`
