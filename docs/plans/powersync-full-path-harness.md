# Plan — Shared full-path benchmark harness: Cairn vs PowerSync
...
The task brief says Cairn's comparable full-path number is "~35k rows/sec at 1k clients." It is not.
...
**Goal.** One harness that runs Cairn and PowerSync back-to-back on the same machine, same Postgres,
...
**Non-goals.**
- No comparison against Cairn's eval-only fan-out ladder (FakeReplicator loopback, 988k ops/sec @ 20k).
...
| 5. Local apply | `Storage::apply_batch` — one txn, upsert rows, advance checkpoint, `tx.commit()` (`crates/cairn-client/src/sqlite.rs:22-24`, commit at `:860`) | checkpoint downloads fully, then `ps_oplog` → `ps_data__<table>`, visible through the `<table>` view |

**Where the finish-line timestamp is taken.** On both sides: at the client's normal read path, after
the row is durably readable, **event-driven on both sides — never a poll.** Cairn — immediately after
...
reason unrelated to PowerSync, while Cairn's side stayed event-driven. If a watch proves unusable in
...
New bin `crates/cairn-bench/src/bin/shared_writer.rs`. Engine-agnostic: it only writes to Postgres.
...
- **`id` must be `TEXT`, not `UUID`.** PowerSync requires a single text-type primary key named `id`
  ...
  handing PowerSync a type-mapping cost Cairn does not pay.
...
### 3.2 Cairn client swarm

Reuse `SyncClient` + `SqliteStorage` unchanged. No lighter apply — a lighter apply would be Cairn
...
those on one host, alongside Postgres, the PowerSync Service, and Cairn, is not a real measurement.

| Lane | Clients | Cairn side | PowerSync side | Reported metric |
|---|---|---|---|---|
| **Apply** (headline) | 1 / 10 / 100 | `SyncClient` + `SqliteStorage` | `@powersync/node`, N processes | rows/sec to local apply, p50/p99, drops |
| **Wire delivery** | 1k / 5k | existing delivered-frames counter (`crates/cairn-bench/src/main.rs:303`) | `test-client concurrent-connections -n` | ops/sec to client wire, drops |
...
1k Cairn clients plus two Postgres instances on one machine. The **5k wire tier is likely infeasible**
...
- **attempted** = rows committed × clients **subscribed at commit time**. The ladder already learned
  ...
  to not-yet-subscribed clients silently inflated the drop rate. "Subscribed" means: Cairn — subscribe
  frame acked; PowerSync — first `checkpoint_complete` received.
- **delivered** = rows readable through the client's normal read path (§2).
- **dropped** = attempted − delivered, attributed. Cairn reports router `dropped` separately from
...
- **apply latency** = `finish_ts − commit_ts`, skew-corrected. Percentiles via `stats.rs::Histogram`
  for Cairn; the Node harness emits raw samples to JSON and reuses the same percentile code so both
...
- Same machine, same run session, **interleaved A/B order** (Cairn, PowerSync, PowerSync, Cairn) so
...
- **Separate Postgres instance for PowerSync bucket storage.** The docs state the bucket storage
  ...
  absorbing the writer load and Cairn's slot. Leaving it there rigs the race against PowerSync.
...
| 1 | `crates/cairn-bench/src/bin/shared_writer.rs` + `race_rows` DDL in `docker/pg-init/` | 1.5 |
| 2 | Cairn apply swarm: `crates/cairn-bench/src/bin/race_cairn.rs` (N × `SyncClient`, per-client histogram, JSON artifact) | 2 |
...
| 4 | Wire-delivery lane: `test-client concurrent-connections` wrapper + Cairn counterpart | 1.5 |
...
product; publishing measurements does not do that. Cairn must not embed, redistribute, or resell the
...
|---|---|---|
| D1 | **The one public claim:** same-stage full-path rows/sec at the *apply finish line*, Cairn vs PowerSync, at a stated client count. | Only honest cross-engine claim (§2). Rejected: "Cairn holds 100k wire clients, PowerSync can't" — that is PowerSync's documented deployment shape (1 API container per ~100 connections, §5.1), not a benchmark result. Wire lane (Step 4) stays secondary and is never quoted as the headline. |
| D2 | **HOLD — planning only.** Execution unblocks when Cairn's own 100k fan-out collapse is *root-caused and documented* in `docs/plans/fanout-100k-collapse-2026-09-02.md` (a fix is not required) **and** the team lead explicitly says go. | The 100k investigation may change which Cairn tiers are interesting. A fix is not a precondition because the apply ladder (D3) tops at 1k, where Cairn shows no collapse. Nothing in §6 runs before then — not Step 0, not the ceiling probe. |
| D3 | **Host: this Mac** (10 cores, Docker Desktop VM 8 GiB). **Apply ladder: 100 / 500 / 1k**, stopping at the first tier that fails D4. | The requested 10k–100k ladder is not runnable on the apply lane on one host: 1k apply already needs ~10 API containers + 1k PowerSync clients + 1k Cairn clients + source PG + MongoDB (§3.4). 5k apply (~50 containers) is out. A rented Linux box was offered and declined for now; if it is used later, *both* engines run on it (same-conditions rule, `docs/BENCHMARK-METHODOLOGY.md`). |
| D4 | **Headroom rule (publish/invalid gate):** for the whole run, host load average < 0.8 × cores **and** no non-harness process > 20 % CPU; otherwise the tier is recorded as *invalid* with its logs (`CONTENDED.md` pattern), never re-rolled silently. | Same threshold that invalidated three Cairn re-measures on 2026-09-02. Mechanically enforced by `race.sh` (Step 5), not judged per tier. Also answers §8 Q2: a tier where the shared source PG saturates fails this rule via the PG process. |
...
| D6 | **Step 0 = MongoDB bucket storage** (single-node replica set, the documented dev/production path) **+ `docker/powersync/config.yaml` rewritten to the documented schema.** | Current file points bucket storage at the *same* Postgres that carries the race writer and Cairn's replication slot — contaminates both numbers. Postgres-on-a-second-instance rejected: supported but not PowerSync's reference path, so any oddity reads as "you misconfigured it". |
