...
```
CAIRN_PUSH_TABLES='tasks;orders:visible@/orders/{id}:New order:Order {id} placed;deliveries:liveactivity:{"status":"{status}","eta_min":"{eta_min}"}'
```
...
payload. `CAIRN_PUSH_TABLES` sets the one key `cairn_route` via `@route`
above; `cairn-pushd`'s `POST /v1/send` takes the whole map:
...
`gcm.*`, and cairn's own `title`, `body`, `category`, `table`, `lsn` — or a
